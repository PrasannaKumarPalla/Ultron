...
the request path, so *any* failure â€” a dropped Ollama connection, a timeout, a
``BrokenPipeError`` when the client went away, or simply unparseable output â€”
...
# PII patterns to strip before storing facts
_PII_PATTERNS = [
    (re.compile(r"\b\d{4}[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}\b"), "[CARD]"),
    (re.compile(r"\b\d{3}-\d{2}-\d{4}\b"), "[SSN]"),
    (re.compile(r"\bpassword[:\s]+\S+", re.IGNORECASE), "[PASSWORD]"),
    (re.compile(r"\btoken[:\s]+[A-Za-z0-9_\-\.]{20,}", re.IGNORECASE), "[TOKEN]"),
    (re.compile(r"\bapi[_\s]?key[:\s]+[A-Za-z0-9_\-]{16,}", re.IGNORECASE), "[API_KEY]"),
]
...
class FactExtractor:
    ...
    def extract(self, user_text: str, assistant_text: str = "") -> List[str]:
        ...
        try:
            ...
            return []
        except Exception:  # noqa: BLE001 â€” extraction must never crash the worker
