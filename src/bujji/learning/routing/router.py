...
from bujji.core.registry import ModelRegistry
from bujji.core.types import RoutingContext
from bujji.learning._stubs import QueryAnalyzer, RouterPolicy

if TYPE_CHECKING:
    from bujji.learning.routing.constraints import ConstraintChecker
...
class HeuristicRouter(RouterPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        # Optional cost/latency/energy gate applied to the candidate list before
        # the routing rules run. When every candidate is filtered out we fall
        # back to the unconstrained list so routing never dead-ends.
    ...
    def select_model(self, context: RoutingContext) -> str:
        ...
        # Constraint gate: drop candidates whose historical cost/latency/energy
        # exceed the configured ceilings, then route over the survivors. If the
        # gate empties the list, keep the original candidates (a constrained
        # route must never leave the user with no model at all).
        if self._constraints is not None:
            filtered = self._constraints.filter(available)
            if filtered:
                available = filtered
