...
Uses GEPA's adapter pattern to bridge Bujji traces into GEPA's
...
from bujji.core.config import GEPAOptimizerConfig
...
class BujjiGEPAAdapter:
    """Implements GEPA's adapter protocol for Bujji agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'bujji[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = BujjiGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "BujjiGEPAAdapter"]
