"""ACE agent optimizer â€” context evolution via the ACE Generator /
Reflector / Curator triad.

ACE (`ace-agent/ace <https://github.com/ace-agent/ace>`_) optimizes
agent *context* â€” an annotated natural-language playbook of strategies
the agent reads at inference time â€” rather than mutating prompts (DSPy)
...
The wrapper adapts Bujji traces into ACE's
...
sidecar overlay under ``~/.bujji/learning/ace/<task>/`` for the
...
from bujji.core.config import ACEOptimizerConfig
from bujji.core.paths import get_config_dir
from bujji.core.registry import LearningRegistry
from bujji.learning._stubs import AgentLearningPolicy

logger = logging.getLogger(__name__)

# Optional dependency â€” ACE is git-installed via the `learning-ace`
...
class _TraceDataProcessor:
    """Adapter that exposes Bujji traces in ACE's three-method API.

    ACE expects a processor with:

    - ``process_task_data(raw)`` â€” normalize a single sample
    - ``answer_is_correct(pred, truth)`` â€” per-sample comparison
    - ``evaluate_accuracy(preds, truths)`` â€” aggregate metric over a list

    We treat each trace as a ``{question, ground_truth_answer}`` pair
    and use the recorded ``feedback`` (0.0â€“1.0) as the ground truth
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Bujji trace records to ACE's sample dict shape."""
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": (
                    "ace not installed (pip install 'bujji[learning-ace]')"
                ),
            }
        ...
        if not playbook_path.exists():
            # ACE completed but didn't write the expected artifact â€” surface
