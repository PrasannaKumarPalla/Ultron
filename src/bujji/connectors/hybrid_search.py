...
Brute-force vector scan is fine at the current corpus size (~5k chunks Ã—
...
from bujji.connectors.embeddings import OllamaEmbedder, decode_embedding
...
class SearchHit:
    ...
    # ``knowledge_chunks.url``. Empty when the source didn't supply one â€” in
...
def _snippet(content: str, max_chars: int = 500) -> str:
    ...
    if len(flat) <= max_chars:
        return flat
    return flat[:max_chars].rstrip() + "â€¦"
...
class HybridSearch:
    ...
    def _build_filters(
        ...
        *,
        ...
        substring of a name or email address is enough â€” handy when the user
    ...
    def search(
        ...
        *,
        ...
        May") â€” in that case only the vector leg runs (and only if an
