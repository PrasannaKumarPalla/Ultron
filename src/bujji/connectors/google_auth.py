...
``~/.bujji/connectors/*.json`` â€” typically a shared ``google.json`` file
...
from bujji.connectors.oauth import load_tokens, save_tokens
...
def call_with_refresh(
    ...
    **kwargs: Any,
    ...
    Any other ``HTTPStatusError`` is re-raised unchanged â€” auth-related retries
    ...
    try:
        ...
        if exc.response is None or exc.response.status_code != 401:
            raise
        logger.info(
            "Google returned 401 on %s â€” refreshing access token and retrying.",
