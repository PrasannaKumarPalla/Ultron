...
class ObsidianConnector(BaseConnector):
    ...
    def search_notes(self, query: str, top_k: int = 10) -> List[Dict[str, str]]:
        """Full-text keyword search across vault notes."""
        if not self.is_connected():
            return []
        q = query.lower()
        results: List[Dict[str, str]] = []
        for doc in self.sync():
            if q in doc.title.lower() or q in doc.content.lower():
                ...
                results.append({"title": doc.title, "url": doc.url or "", "snippet": snippet})
    ...
    def write_note(
        ...
        """Create or overwrite a note in the vault. Returns the vault-relative path."""
        if not self.is_connected():
            raise RuntimeError("Obsidian vault not configured or not found")

        subfolder = folder or "Bujji"
        note_dir = Path(self._vault_path) / subfolder
        note_dir.mkdir(parents=True, exist_ok=True)

        # Sanitise filename
        safe_name = "".join(c if c.isalnum() or c in " -_()" else "_" for c in title).strip()
        fpath = note_dir / f"{safe_name}.md"
        ...
        return str(fpath.relative_to(self._vault_path))

    def append_note(self, title: str, content: str, folder: Optional[str] = None) -> str:
        ...
        if fpath.exists():
            ...
            self.write_note(title, content, folder=folder)
        ...
        return [
            ...
            ToolSpec(
                ...
                description=(
                    ...
                    "Use this to save facts, summaries, research results, or anything the user wants to remember."
                ),
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "folder": {
                            "type": "string",
                            "description": "Subfolder inside the vault (default: Bujji)",
                            "default": "Bujji",
                        },
                        "tags": {
                            "type": "array",
            ...
            ToolSpec(
                ...
                description=(
                    "Append text to an existing Obsidian note, or create it if it doesn't exist. "
                    "Useful for daily journals, running logs, or accumulating research."
                ),
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "folder": {"type": "string", "description": "Subfolder (default: Bujji)", "default": "Bujji"},
                    },
                    "required": ["title", "content"],
                },
                category="knowledge",
