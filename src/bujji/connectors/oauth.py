...
from bujji.core import open_browser
...
class OAuthProvider:
    ...
    # Filenames in ~/.bujji/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.bujji/connectors/`` and falls
    back to environment variables ``BUJJI_{NAME}_CLIENT_ID`` and
    ``BUJJI_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"BUJJI_{provider.name.upper()}"
...
def refresh_google_token(path: str) -> Optional[str]:
    ...
    fails (network error or Google returns a non-2xx response â€” typically
        ...
        def do_GET(self) -> None:  # noqa: N802 â€” required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Bujji.</p>"
                    b"</body></html>"
                )
