...
Uses OAuth 2.0 tokens stored locally (see :mod:`bujji.connectors.oauth`).
...
from bujji.connectors.google_auth import (
    GoogleAuthError,
)
from bujji.connectors.google_auth import (
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `bujji connect gmail` short-circuit with
    ...
    def sync(
        ...
        *,
        ...
        """
        # Existence check only â€” the actual access token is reloaded on every
        ...
        while True:
            ...
            for msg_stub in messages:
                ...
                # Raw header values, exactly as Gmail returned them â€” preserved
                ...
                doc = Document(
                    ...
                    # resolves directly â€” so citations have a working URL
    ...
    def _refresh_token(self) -> str:
        ...
        refresh token has been revoked â€” user must re-authorise).
        """
        new = refresh_google_token(self._credentials_path)
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed â€” re-run `bujji connect gmail`"
            )
        return new
