...
sync sees everything the user can see â€” including their 1:1 DMs and
...
from bujji.connectors._stubs import BaseConnector, Document, SyncStatus
from bujji.connectors.oauth import delete_tokens, load_tokens, save_tokens
from bujji.core.config import DEFAULT_CONFIG_DIR
from bujji.core.registry import ConnectorRegistry
from bujji.tools._stubs import ToolSpec
...
# User Token Scopes â€” Slack distinguishes user_scope (xoxp-) from scope
...
def _slack_api_conversations_list(
    ...
    *,
    ...
    """
    # Include every conversation type the bot can list â€” public + private
    # channels, multi-person DMs, and 1:1 DMs â€” so a "connect and sync"
...
def _slack_api_auth_test(token: str) -> Dict[str, Any]:
    ...
    construct message permalinks â€” the workspace subdomain isn't carried
...
def _validate_user_token(token: str) -> None:
    ...
    explicitly because a bot can only see DMs *to/from itself* â€” paste a
...
def _team_domain_from_auth(auth_resp: Dict[str, Any]) -> str:
    ...
    workspace ``url`` is missing â€” losing the workspace breaks permalinks
...
class SlackConnector(BaseConnector):
    ...
    def auth_url(self) -> str:
        ...
        OAuth Token (``xoxp-``) â€” bot tokens (``xoxb-``) can't see human-
    ...
    def handle_callback(self, code: str) -> None:
        ...
        1. **Shape** â€” must start with ``xoxp-`` (``xoxb-`` bot tokens and
           any other prefix are rejected via :func:`_validate_user_token`).
        2. **Liveness** â€” a live ``auth.test`` call must return ``ok`` so an
        ...
        except Exception as exc:  # noqa: BLE001 â€” surface as a token error
            raise SlackTokenError(
    ...
    def sync(
        ...
        *,
        ...
        ``conversations.list`` already reflects what the user can see â€”
        ...
        # Step 0: resolve workspace context â€” the subdomain is needed for
        ...
        for channel in all_channels:
            ...
            # Display name: IMs have no ``name`` field â€” render as
            ...
            while True:
                ...
                for msg in messages:
                    ...
                    # one, else the display name â€” matches the Gmail
