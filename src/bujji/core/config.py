...
User configuration lives at ``~/.bujji/config.toml``.  ``load_config()``
...
# once at import via the env-aware resolver in ``bujji.core.paths`` (the
# install-script model: ``BUJJI_HOME`` / ``XDG_DATA_HOME`` are set before
...
def _ensure_config_dir() -> Path:
    ...
    from bujji.security.file_utils import secure_mkdir
...
_WINDOWS_VIRTUAL_GPU_MARKERS = (
    "virtual", "hyper-v", "basic display", "basic render", "indirect",
    "citrix", "vmware", "parallels", "rdp", "kvm", "qxl",
)
# Discrete-card name hints — distinguishes "AMD Radeon RX 7900 XTX" (dGPU)
# from "AMD Radeon(TM) Graphics" (integrated iGPU sharing system RAM).
_DGPU_NAME_PATTERN = re.compile(
    r"\brx\s*\d{3,4}\b|\barc\s+[ab]\d{3}\b|\b(?:rtx|gtx)\s*\d+"
    r"|\bquadro\b|\bfirepro\b|radeon pro|radeon vii",
    re.IGNORECASE,
)
_VENDOR_HINTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("nvidia", ("nvidia", "geforce", "rtx", "gtx", "quadro")),
    ("amd", ("amd", "radeon", "ati ")),
    ("intel", ("intel", "arc ", "uhd graphics", "iris")),
)
...
def _read_winreg_vram_bytes(key, reg=None) -> Optional[float]:
    """Exact adapter memory from the display-class registry key.

    ``AdapterRAM`` in WMI is capped near 4 GiB (32-bit legacy), so cards like
    the 24 GB RX 7900 XTX lie about themselves there.  The driver writes the
    true byte size here — either as REG_QWORD (an int) or REG_BINARY whose
    first bytes little-endian-encode it.
    """
    try:
    ...
    for value_name in ("HardwareInformation.qwMemorySize", "HardwareInformation.MemorySize"):
        try:
            data, _value_type = reg.QueryValueEx(key, value_name)
        except OSError:
            continue
...
def _win_registry_adapters() -> List[Dict[str, Any]]:
    """Enumerate installed display adapters from the device-class registry."""
    out: List[Dict[str, Any]] = []
    try:
        import winreg  # type: ignore[import-not-found]

        base_key = r"SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}"
        root = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, base_key)
    ...
    with root:
        ...
        while True:
            ...
            with key:
                try:
                    name, _value_type = winreg.QueryValueEx(key, "DriverDesc")
                except OSError:
                    continue
                out.append({"name": str(name), "vram_bytes": _read_winreg_vram_bytes(key)})
    return out


def _select_windows_gpu(adapters: List[Dict[str, Any]]) -> Optional[GpuInfo]:
    """Pick the best display adapter from an adapter list (pure, unit-testable)."""
    candidates = []
    for adapter in adapters or []:
        ...
        if not name:
            continue
        low = name.lower()
        if any(marker in low for marker in _WINDOWS_VIRTUAL_GPU_MARKERS):
            continue
        candidates.append({"name": name, "vram_bytes": adapter.get("vram_bytes")})
    if not candidates:
        return None

    discretes = [a for a in candidates if _DGPU_NAME_PATTERN.search(a["name"])]
    chosen = discretes[0] if discretes else max(candidates, key=lambda a: a["vram_bytes"] or 0)
    vram_bytes = chosen["vram_bytes"]
    if not vram_bytes:
        # iGPUs often report 0 dedicated memory; borrow the largest known figure.
        vram_bytes = max((a["vram_bytes"] or 0) for a in candidates) or 0
    return GpuInfo(
        vendor=_vendor_for_adapter(chosen["name"]),
        name=chosen["name"],
        vram_gb=round(float(vram_bytes) / (1024**3), 1),
        compute_capability="",
        count=max(len(discretes), 1),
    )
...
def _detect_windows_gpu(
    ...
    """Detect a display adapter without any vendor CLI (AMD dGPUs on Windows).

    NVIDIA ships ``nvidia-smi``, ROCm hosts have ``rocm-smi`` — but a Windows
    box running an RX 7900 XTX has neither, so read the display-class registry
    instead (stdlib ``winreg``, no subprocess, exact VRAM sizes).
    """
    if (system or platform.system()) != "Windows":
        return None
    if adapters is None:
        adapters = _win_registry_adapters()
    return _select_windows_gpu(adapters)
...
_VERIFIED_OLLAMA_DOWNLOAD_GB = {
    # OCI layer totals from registry.ollama.ai, verified 2026-08-14.
    "qwen3.5:2b": 2.55,
    "qwen3.5:4b": 3.16,
    "qwen3.5:9b": 6.14,
    "qwen3.5:27b": 16.22,
...
def estimated_download_gb(parameter_count_b: float, model_id: str = "") -> float:
    """Return a verified download size when known, otherwise a Q4 estimate."""
    if model_id in _VERIFIED_OLLAMA_DOWNLOAD_GB:
        return _VERIFIED_OLLAMA_DOWNLOAD_GB[model_id]
...
class IntelligenceConfig:
    ...
    default_model: str = ""
    fallback_model: str = ""
    complex_model: str = ""  # escalation for hard/tool-heavy queries
    indic_model: str = ""  # model for Telugu/Indic-language queries
...
class RoutingLearningConfig:
    ...
    # First-class routing constraints (None = not enforced). Compared against a
    # model's historical telemetry averages before selection; a candidate that
    # exceeds a ceiling is dropped in favour of a cheaper/local fallback.
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e bujji[learning-ace]`` once the
    ...
    # most Bujji users have credentials for.
    ...
    # Empty string defaults to ``~/.bujji/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "bujji"
...
class StorageConfig:
    ...
    # Keep injected memory context SMALL — every prompt pays for it in
    # time-to-first-token (the local model must read the whole context before
    # replying). 2048 tokens of context added ~5-9s latency per turn.
    context_top_k: int = 3
    ...
    # the background and persists them across sessions (see bujji.memory).
...
def _default_system_prompt() -> str:
    """Build the default system prompt naming the *active* brand.

    Derives the assistant's name from ``get_branding()`` so a white-label build
    says "Your Assistant" while the shipped Bujji brand says "Bujji" — instead of
    hardcoding a name that drifts from the brand (see #540: anti-identity-drift).
    """
    try:
        from bujji.brand import get_branding

        name = get_branding().product_name
    except Exception:
        name = "your assistant"
    return (
        f"You are {name}, a helpful AI assistant running locally on the "
        ...
        f"who or what you are, identify yourself as {name}. Respond "
...
class AgentConfig:
    ...
    # Keep listening for follow-ups after each spoken reply.
    conversation_mode: bool = True
    startup_briefing: bool = True  # speak a greeting/weather/status checklist on launch
    user_name: str = ""  # how the assistant addresses the user in greetings
    location: str = ""  # city for weather in the briefing (IP geolocation if empty)
    default_system_prompt: str = field(default_factory=_default_system_prompt)
...
class AnalyticsConfig:
    ...
    usage events sent to the Bujji team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.bujji/whatsapp_auth
    assistant_name: str = "Bujji"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "bujji-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.bujji/scheduler.db
...
class TTSLanguageConfig:
    ...
    backend: str = ""   # override backend for this language
    voice_id: str = ""  # override voice for this language
...
class TTSConfig:
    ...
    voice_id: str = ""
    speed: float = 1.0
    # language-specific overrides keyed by BCP-47 tag (e.g. "te", "hi", "ja")
    languages: Dict[str, TTSLanguageConfig] = field(default_factory=dict)

    def for_language(self, lang: str) -> "TTSConfig":
        """Return a copy of this config with language-specific overrides applied."""
        if lang and lang in self.languages:
            override = self.languages[lang]
            import copy
            merged = copy.copy(self)
            if override.backend:
                merged.backend = override.backend
            if override.voice_id:
                merged.voice_id = override.voice_id
            return merged
        return self
...
class SkillsConfig:
    ...
    index_repo: str = "https://github.com/bujji/skill-index.git"
    index_dir: str = field(
...
@dataclass
class ObsidianConnectorConfig:
    """Config for the local Obsidian / Markdown vault connector."""

    enabled: bool = False
    vault_path: str = ""
    notes_folder: str = "Bujji"


@dataclass
class ConnectorsConfig:
    """Config container for all data-source connectors."""
...
@dataclass
class BujjiConfig:
    """Top-level configuration for Bujji."""
...
# Sections that users may set via ``bujji config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(BujjiConfig.__dataclass_fields__.keys()) - {
    "hardware",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``BujjiConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "BujjiConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> BujjiConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``BUJJI_CONFIG`` when set,
        otherwise ``~/.bujji/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = BujjiConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("BUJJI_CONFIG"):
        config_path = Path(os.environ["BUJJI_CONFIG"]).expanduser().resolve()
    else:
    ...
    if config_path.exists():
        ...
        # tts.languages: raw dicts â†’ TTSLanguageConfig instances
        for _lang_key, _lang_val in list(cfg.tts.languages.items()):
            if isinstance(_lang_val, dict):
                cfg.tts.languages[_lang_key] = TTSLanguageConfig(
                    backend=_lang_val.get("backend", ""),
                    voice_id=_lang_val.get("voice_id", ""),
                )
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``bujji init``)
# ---------------------------------------------------------------------------
...
def generate_minimal_toml(
    ...
    *,
    ...
    model = recommend_model(hw, engine) if model is None else model
    gpu_comment = ""
    if hw.gpu:
...
# Full reference config: bujji init --full
...
def generate_default_toml(
    ...
    *,
    ...
    """Render a commented TOML string suitable for ``~/.bujji/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine) if model is None else model
    gpu_line = ""
    if hw.gpu:
...
# background and persists them. Starts/stops with `bujji serve` and
# `bujji chat`; manage stored facts with `bujji memory list` / `clear`.
[memory]
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.bujji/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.bujji/whatsapp_auth
