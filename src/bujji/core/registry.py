...
if TYPE_CHECKING:
    ...
    from bujji.memory.store import FactStore
...
class AgentRegistry(RegistryBase[Type["BaseAgent"]]):
    ...
    @classmethod
    def by_mode(cls, mode: Any) -> Tuple[Tuple[str, Type["BaseAgent"]], ...]:
        """Return ``(key, agent_cls)`` pairs whose ``execution_mode`` matches.

        *mode* may be an ``ExecutionMode`` or its string value. Agents that
        predate the attribute default to on-demand via ``BaseAgent``.
        """
        from bujji.agents._stubs import ExecutionMode

        wanted = ExecutionMode(mode) if not isinstance(mode, ExecutionMode) else mode
        return tuple(
            (key, agent_cls)
            for key, agent_cls in cls._entries().items()
            if getattr(agent_cls, "execution_mode", ExecutionMode.ON_DEMAND) == wanted
        )
...
class MinerRegistry(RegistryBase[Any]):
    """Registry for miner / sidecar node providers (e.g. Pearl/Olas)."""
