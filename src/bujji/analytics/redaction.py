...
(:mod:`bujji.analytics.events` is the structural half).
...
_PII_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    # IPv6 (loose â€” any colon-separated hex)
...
def redact(properties: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in properties.items():
        if isinstance(value, str):
            if not value:
                # empty string is uninformative â€” drop
                continue
            ...
            # Composite values are never sent â€” keeps the surface tiny.
            continue
        out[key] = value
    return out
...
def hash_id(s: str) -> str:
    ...
    allowlist â€” we still want to see "uses-a-custom-model-X" cohorting
