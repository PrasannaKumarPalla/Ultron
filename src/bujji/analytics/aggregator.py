"""Per-session aggregator â€” turns many internal events into one analytics event.

Without this, a single chat (50 inferences, 10 tool calls) would
produce ~60 PostHog events. With it, the same chat produces one
``chat_session_ended`` event with summary properties â€” a ~60Ã— reduction
...
class SessionAggregator:
    ...
    def end_session(self, session_id: str) -> None:
        ...
        if stats is None or stats.inference_count == 0:
            # Nothing meaningful happened â€” don't emit a no-op event.
            return
        self._emit(stats)
