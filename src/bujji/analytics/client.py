...
A thin adapter over the official ``posthog`` SDK that:
  ...
  - Fails silently â€” analytics must never break the host application.
...
from bujji.analytics.events import validate_event
from bujji.analytics.identity import (
    get_or_create_anon_id,
    is_analytics_enabled,
)
from bujji.analytics.redaction import redact
...
class AnalyticsClient:
    ...
    def _init_sdk(self) -> None:
        try:
            ...
            logger.debug(
                "PostHog analytics initialised host=%s anon_id=%sâ€¦",
                self.config.host,
                self.anon_id[:8],
            )
    ...
    def capture(
        ...
        Runs through redaction â†’ event-spec validation â†’ SDK capture.
