...
The internal bus (:mod:`bujji.core.events`) carries dozens of
event types â€” most are too granular or too internal to ship as
analytics. The bridge:
  ...
  - Tracks first-uses in-process so ``tool_first_used`` fires once per
    ...
    not promised â€” that would require disk state and isn't worth the
    complexity for v1.)
"""
...
from bujji.analytics.redaction import hash_id
from bujji.core.events import Event, EventType
...
# Allowlist of known engines â€” anything else gets normalised to "unknown"
...
class EventBridge:
    ...
    def _on_inference_end(self, event: Event) -> None:
        try:
            ...
            # survives server restarts â€” not just per process lifetime).
            with self._lock:
    ...
    def _on_tool_end(self, event: Event) -> None:
        try:
            ...
            if first:
                ...
                from bujji.analytics.events import KNOWN_TOOL_NAMES
    ...
    def _on_agent_turn_end(self, event: Event) -> None:
        ...
        # rather than a hard close â€” the aggregator will flush on idle
