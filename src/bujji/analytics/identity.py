...
events tie back to the same person across the installâ†’first-run funnel.

No email, no name, no hardware fingerprint â€” just an opaque UUID.
"""
...
def get_or_create_anon_id(path: Path | str) -> str:
    ...
    Idempotent across processes â€” if the file already exists with a
