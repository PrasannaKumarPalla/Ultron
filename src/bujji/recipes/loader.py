...
Recipes are the universal composition format for Bujji.  Each recipe
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Bujji.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Bujji constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.bujji/recipes/`` directory
    4. User ``~/.bujji/operators/`` directory
