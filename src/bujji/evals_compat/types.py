...
class RunConfig:
    ...
    # the BujjiConfig.agent.max_turns value (typically 10). Set higher when
    # running thinking/reasoning models that consume turns on intermediate
    # reasoning before producing tool calls.
    max_turns: Optional[int] = None
    # Spec Â§6.2: backend-external endpoint config (for hermes/openclaw)
    ...
    # global_agent_timeout_sec bounds each trial's agent phase â€” SETUP+RUN
...
class ExecutionConfig:
    ...
    # to BujjiConfig.agent.max_turns (default 10). Bump to 30-50 for
...
class EvalSuiteConfig:
    ...
    # Spec Â§6.2: optional [backend.external] section for hermes/openclaw
