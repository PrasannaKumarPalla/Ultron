...
from bujji.core.types import TOKEN_COUNTING_VERSION, Message, TelemetryRecord
from bujji.engine._stubs import InferenceEngine, StreamChunk
from bujji.telemetry.gpu_monitor import GpuSample
...
class InstrumentedEngine(InferenceEngine):
    ...
    :class:`~bujji.telemetry.energy_monitor.EnergyMonitor`), it is
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Wall-clock t0 for the record timestamp; perf_counter for durations.
        # time.time() has ~15ms resolution on Windows, so fast calls would
        # measure a 0s latency (zeroing throughput and the energy split).
        t0 = time.time()
        ...
        latency = time.perf_counter() - perf0
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        t0 = time.time()  # wall-clock, for the record timestamp
        perf0 = time.perf_counter()  # high-res, for durations (see generate())
        ...
        latency = time.perf_counter() - perf0
        ttft = token_timestamps[0] - perf0 if token_timestamps else 0.0
