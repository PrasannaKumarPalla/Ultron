...
HTTP API, configured to use Bujji's local engine through an
OpenAI-compatible provider. This keeps coding-agent work local-first: opencode
handles the agentic loop / tools, Bujji supplies the model.
...
def _derive_openai_base_url(engine: Any) -> str:
    """Best-effort OpenAI-compatible base URL for an Bujji engine.

    HTTP engines (Ollama, vLLM, llama.cpp, SGLang, LM Studio, â€¦) expose a
...
def _extract_tool_results(parts: List[dict]) -> List[ToolResult]:
    """Map opencode ``tool`` parts to Bujji ToolResults (best-effort)."""
...
class OpenCodeAgent(BaseAgent):
    ...
    inference runs on Bujji's selected local model. ``agent`` selects
    ...
    def _default_permission(self) -> dict:
        ...
        asks before bash) â€” that would block forever with no TTY. We set
    ...
    def _ensure_server(self) -> str:
        ...
        self._config_dir = tempfile.mkdtemp(prefix="bujji-opencode-")
