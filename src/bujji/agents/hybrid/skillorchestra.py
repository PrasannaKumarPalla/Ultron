"""SkillOrchestraAgent â€” inference-time router (Wang et al., 2026).

Paper: arXiv:2602.19672. The published pipeline is four phases â€” explore
...
agent that maximizes weighted competence minus Î»Â·cost.
...
scope for the Bujji port (and was out of scope in the hybrid harness).
...
= 0.500 acc, $0.02/task â€” 30Ã— cheaper than baseline-cloud (0.567 / $0.66)
...
from bujji.agents.hybrid._base import LocalCloudAgent
from bujji.agents.hybrid._prices import supports_temperature
...
# typical Qwen3.5-27B-FP8 vs Opus 4.7 behavior â€” what a 1-iteration learn
...
class SkillOrchestraAgent(LocalCloudAgent):
    ...
    def _is_soft_failure(self, exc: BaseException) -> Optional[str]:
        # Empty/unbalanced router JSON â€” treat as soft failure to match the
