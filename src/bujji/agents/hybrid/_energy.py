...
integrates power Ã— dt â†’ joules across the cell's wall-time. Used as a
context manager wrapping ``_run_cell_locked`` so the whole cell's GPU
energy is attributed to the cell (not per-task â€” concurrency makes
...
* Samples **all visible NVIDIA GPUs on the host where the runner runs**.
  ...
  with vLLM on a remote box), the collector logs and yields 0 â€” set
  ``BUJJI_HYBRID_ENERGY=0`` to silence the warning.
* ``CUDA_VISIBLE_DEVICES`` is *honored* for parity with vLLM: only those
  GPU indices are sampled. Unset = all GPUs on the host.
* **Cloud energy is not measured** â€” cloud calls go over HTTPS to
  ...
  uncertain â€” leaving as a TODO until we explicitly decide to estimate.
"""
...
class EnergyCollector:
    ...
    def __init__(self, sample_hz: float = 2.0) -> None:
        ...
        self._t0: float = 0.0
        self._t1: float = 0.0
        self._enabled = os.environ.get("BUJJI_HYBRID_ENERGY", "1") != "0"
        self._pynvml = None
        self._handles: list = []
    ...
    def __enter__(self) -> "EnergyCollector":
        ...
        try:
            ...
            self._pynvml = pynvml
        except Exception as e:  # noqa: BLE001 â€” NVML failures must never crash the run
            _log_once(
                f"NVML unavailable ({type(e).__name__}: {e}); "
                "energy_j_total will be 0. Set BUJJI_HYBRID_ENERGY=0 to silence."
            )
            self._pynvml = None
            return self
    ...
    def _sample_loop(self) -> None:
        """Trapezoid-integrate Î£_gpu power_W Ã— dt over the cell run.
        ...
        while not self._stop.is_set():
            ...
            except Exception:  # noqa: BLE001 â€” keep going even on transient NVML errors
                self._stop.wait(period)
                continue
            now = time.time()
            if last_total_w is not None:
                dt = now - last_t
                # Trapezoid rule: mean power across the interval Ã— dt.
