"""LocalCloudAgent â€” shared base for hybrid local+cloud paradigm agents.
...
Why not just use Bujji's :class:`InferenceEngine` for both? Two reasons:
...
2. Cloud-side quirks (Opus 4.7 temperature stripping, GPT-5 family
   ``max_completion_tokens``) are paradigm-shaped â€” Minions needs structured
...
kwargs (``local_model``, ``local_endpoint``, ``cloud_endpoint``, â€¦) follow.
"""
...
from bujji.agents._stubs import AgentContext, AgentResult, BaseAgent
from bujji.agents.hybrid._openai_retry import patch_openai_globally
...
# loop, archon, â€¦) inherits the hardening without each call site having
...
# $0.01/call number â€” kept as a separate constant so it can drift.
...
def tavily_search_context(
    ...
    *,
    ...
    """Run Bujji WebSearchTool and return accounting-friendly metadata."""
...
# triggering 429s that the OpenAI SDK retries with backoff â€” wasted latency
# and noisy traces.
#
...
#
# - **Concurrency** â€” a ``threading.Semaphore``. Default 20 in-flight calls;
...
# - **RPM** â€” a sliding window of timestamps in a ``deque``. Default 60
...
# are NOT rate-limited here â€” OpenAI and Anthropic have their own concurrency
...
def _jsonable(v: Any) -> Any:
    """Best-effort JSON-friendly conversion. Pydantic models â†’ .model_dump(),
...
class LocalCloudAgent(BaseAgent):
    ...
    - ``engine``, ``model``: the cloud engine + model id (satisfies
      :class:`BaseAgent`'s contract; only used incidentally â€” we make raw
      SDK calls).
    - ``local_model``, ``local_endpoint``: vLLM-served local model and its
      OpenAI-compatible endpoint, e.g. ``"http://localhost:8001/v1"``.
    - ``cloud_endpoint``: ``"anthropic"`` or ``"openai"`` â€” picks the
      cloud SDK.
    - ``cfg``: paradigm-specific knobs (max_tokens, schemas, mode, â€¦).
    """
    ...
    # ------------------------------------------------------------------
    # SDK call helpers â€” raw clients, paradigm-shaped quirks applied
    # ------------------------------------------------------------------
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the Bujji-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/bujji/engine/cloud.py``) â€” we strip it here.

        Note: unlike ``_call_openai``, we do NOT apply the GPT-5 family
        ``max_completion_tokens`` rewrite or temperature stripping â€”
    ...
    def _call_gemini(
        ...
        *,
        ...
        Tool-call parity with Anthropic is intentionally NOT implemented â€”
    ...
    def _call_vllm(
        ...
        *,
        ...
        Captures the full response into the trace â€” including any tool_calls
    ...
    def _call_anthropic_agent(
        ...
        *,
        ...
        inside a single message and don't require a tool_result echo â€”
        ...
        for turn in range(max(1, max_turns)):
            ...
            # here â€” break and let the caller (or future loop variant)
            ...
            # (server side) â€” Anthropic returned mid-thought. Append the
    ...
    def _call_openai_agent(
        ...
        *,
        ...
        Returns ``(final_text, prompt_tokens_sum, completion_tokens_sum,
        n_web_searches_sum, turns)`` â€” the same 5-tuple shape as
        ...
        so ``turns`` is 1 â€” the ``max_turns`` arg is accepted only for
    ...
    def _call_gemini_agent(
        ...
        *,
        ...
        Returns ``(final_text, prompt_tokens_sum, completion_tokens_sum,
        n_web_searches_sum, turns)`` â€” same shape as ``_call_anthropic_agent``.
        ...
        â€” we count the ``web_search_queries`` Gemini reports, falling back to
    ...
    def _call_cloud(
        ...
        **kwargs: Any,
        ...
        is discarded â€” paradigms that care should call ``_call_anthropic``
        directly.
        """
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Subclasses don't track this themselves â€” every SDK call site
    ...
    def record_trace_event(event: Dict[str, Any]) -> None:
        ...
        (Minions's protocol loop, Archon's layer pipeline, â€¦) to drop a
