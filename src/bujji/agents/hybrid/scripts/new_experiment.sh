...
# src/bujji/agents/hybrid/registry/<method>.toml
# (registry is split by method â€” minions.toml, conductor.toml, etc.).
#
# Usage:
...
#
# Then run:
#   python -m bujji.agents.hybrid.runner --cell <printed name>
...
# Map shortnames â†’ full HF id + endpoint
case "$local_model" in
...
if [[ ! -f "$registry_file" ]]; then
    echo "no registry file for method '${method}' â€” expected $registry_file" >&2
    echo "known: $(ls "$REGISTRY_DIR" | tr '\n' ' ')" >&2
    exit 2
fi

# Method-specific method_cfg defaults â€” keep simple, override later in TOML.
case "$method" in
    minions)
...
echo "Added cell: ${name}  â†’  $(realpath --relative-to="$(pwd)" "$registry_file")"
echo "Run with: python -m bujji.agents.hybrid.runner --cell ${name}"
