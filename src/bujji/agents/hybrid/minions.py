"""MinionsAgent â€” port of HazyResearch Minions protocol.
...
- ``"minion"`` â€” single local worker, one cloud supervisor (cheaper).
  Default.
- ``"minions"`` â€” parallel local workers, cloud aggregator.

Hybrid harness result: ``minions-swebenchverified-qwen27b-opus-500`` =
0.274 acc / $0.09 per task â€” beats baseline-cloud's 0.236 / $0.95 on
...
``uv pip install -e path/to/minions``). Import is lazy â€” the agent class
...
- Inject ``timeout=600``/``max_retries=5`` defaults into
  ``anthropic.Anthropic()`` â€” Minions builds bare clients which 60s-timeout
...
from bujji.agents.hybrid._openai_retry import (
    patch_openai_globally as _patch_openai_globally,
)
from bujji.agents.hybrid._prices import NO_TEMP_PREFIXES, default_max_output_tokens
from bujji.agents.hybrid.mini_swe_agent import run_swe_agent_loop
...
MINIONS_SWE_PLANNER_SYS = (
    ...
    "relevant, and 1-3 concrete approaches it should try. Be specific â€” "
...
def _minions_turn_schema(kwargs: Dict[str, Any]) -> Dict[str, Any]:
    """Pick the schema by sniffing the prompt â€” see module docstring."""
    blob_parts = [str(kwargs.get("system", ""))]
    for msg in kwargs.get("messages", []) or []:
        content = msg.get("content", "") if isinstance(msg, dict) else ""
        blob_parts.append(str(content))
...
def _stub_missing_imports() -> None:
    ...
    Two annoyances:
    1. ``mistralai`` 2.x dropped top-level ``Mistral`` â†’ ImportError.
    ...
    for cls in (_msgs_mod.Messages, _beta_msgs_mod.Messages):
        ...
        def make_patched(orig):  # type: ignore[no-untyped-def]
            def patched(self, **kwargs):  # type: ignore[no-untyped-def]
                ...
                # â€” cache_control belongs on individual content blocks, not on
...
def _patch_gemini_client_usage() -> None:
    ...
    1. ``response.usage_metadata.candidates_token_count`` is sometimes ``None``
       ...
       does ``total_token_count - candidates_token_count`` raw â†’ ``TypeError``.
    ...
    on the value-extraction lines â€” the API call itself is unchanged.
    """
    ...
    def patched_schat(self, messages, **kwargs):  # type: ignore[no-untyped-def]
        ...
        # Skip the OpenAI-compat branch â€” Minions paradigm never sets that.
        ...
        # Defensive text accessor â€” upstream `response.text` can raise when
...
def _apply_patches_once() -> None:
    ...
    # Idempotent â€” also applied at ``_base`` import time.
...
def _prefetch_context(
    ...
    Minions's premise is "worker reads a doc, asks cloud for help" â€” but GAIA
    ...
    and zeros â€” the protocol still runs.
    """
    out: Dict[str, Any] = {
    ...
    try:
        ...
        from bujji.agents.hybrid._prices import cost as _cost_usd
        out.update(
            text=text,
            tokens=p + c,
    ...
    def _run_paradigm(
        ...
        **kwargs: Any,
        ...
        if self._cloud_endpoint == "openai":
            ...
            # have to hand it a GeminiClient instance â€” no extra plumbing.
        ...
        #   - omitted          â†’ prefetch ON (legacy default for minions GAIA)
        #   - enabled = false  â†’ prefetch OFF
        #   - enabled = true   â†’ prefetch ON (honors max_uses)
        ...
        # auto-trace missed every turn. Record the protocol output directly â€”
        ...
        meta = {
            ...
            # The Minions protocol itself is supervisorâ†”worker text, no tools.
