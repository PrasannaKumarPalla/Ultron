"""SkillOrchestraAgent â€” the Bujji harness entry point.
...
* **Learned handbook** â€” produced offline by the explore->learn->select
...
* **FAISS wiki retriever** â€” the ``search`` tool POSTs to it when
  ``method_cfg.retriever_url`` is set; otherwise it falls back to
  Anthropic ``web_search``.
* **6+ model pool** â€” the alias tiers collapse onto the cell's local +
...
class SkillOrchestraAgent(LocalCloudAgent):
    ...
    def _run_paradigm(
        ...
        **kwargs: Any,
        ...
        # QA path â€” the faithful eval orchestrator.
        ...
        if handbook is None and strategy in ("router_decides", "analyze_model_decide"):
            ...
            # pick â€” they need no learned skill data, so an empty handbook
