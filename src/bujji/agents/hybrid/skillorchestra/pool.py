...
maps stage aliases â€” ``search-1/2/3``, ``reasoner-1/2/3``,
``answer-1/2/3/4``, ``answer-math-1/2`` â€” onto a pool of 6+ models served
via SGLang. Bujji runs a 2-model world (one local vLLM student + one
...
def call_alias(
    ...
    *,
    ...
    defaults to 1.0 â€” the value the original ``call_tool`` uses for every
