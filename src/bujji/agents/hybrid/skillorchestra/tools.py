...
Faithful port of ``orchestration/eval_frames.py:call_tool`` â€” same worker
prompts, same extraction, same Python subprocess execution. Two deltas,
both forced by the Bujji environment and documented inline:

* ``search`` â€” the original POSTs to a FAISS wiki retriever service. We
...
* in-tool correctness check â€” the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Bujji
...
# ---------------------------------------------------------------------------
# Tool schemas â€” orchestration/tools.json, in Anthropic + OpenAI shapes.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# enhance_reasoning / code  â€”  eval_frames.py:659-812
# ---------------------------------------------------------------------------
...
def run_code(
    ...
    *,
    ...
    rather than raising â€” the orchestrator learns the model can't code.
    """
    prompt = (
        context_str.strip() + "\n\n"
...
# ---------------------------------------------------------------------------
# answer  â€”  eval_frames.py:814-997
# ---------------------------------------------------------------------------
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped â€” Bujji scores
    downstream.
    """
...
# ---------------------------------------------------------------------------
# search  â€”  eval_frames.py:999-1096
# ---------------------------------------------------------------------------
...
def run_search(
    ...
    *,
    ...
    Bujji substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip() + "\n\n"
        + f"Question: {problem}\nInstead of directly answering the question, "
    ...
    if search_backend == "tavily":
        ...
        # Faithful path â€” the original FAISS retriever service.
        import requests
        ...
        # Substitution path â€” server-side web search via the cloud's
        ...
        # the orchestrator answers the GAIA question blind â€” fail loud.
        ...
        if endpoint not in _SEARCH_CAPABLE_ENDPOINTS:
            raise ValueError(
                ...
                "override the search-* aliases in method_cfg.model_pool â€” "
