...
::
    ...
    python -m bujji.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3

Reads a cell definition from ``registry/<method>.toml`` (bundled with this
package or pointed at by ``BUJJI_HYBRID_REGISTRY_DIR``), constructs
the registered agent, loads bench tasks via Bujji's existing dataset
...
existing rescore / dashboard scripts can read Bujji cells without
modification.
"""
...
from bujji.agents._stubs import AgentContext, AgentResult
from bujji.agents.hybrid._energy import EnergyCollector
from bujji.agents.hybrid._prompts import format_prompt as _format_prompt
...
# blocked indefinitely â€” and with the runner's ThreadPoolExecutor that
...
# ``BUJJI_HYBRID_TASK_TIMEOUT_S`` (0 / negative disables).
DEFAULT_TASK_TIMEOUT_S = float(
    os.environ.get("BUJJI_HYBRID_TASK_TIMEOUT_S", "1800") or 1800
)
...
def load_registry(registry_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    env_override = os.environ.get("BUJJI_HYBRID_REGISTRY_DIR")
    if env_override:
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    from bujji.evals_compat.gaia import GAIADataset
    ...
    for rec in ds.iter_records():
        ...
        # Bujji prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _load_swebench_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    from bujji.evals_compat.swebench import SWEBenchDataset
...
def _load_subset_file(subset_path: str) -> Dict[str, Any]:
    ...
    Resolution order:
      ...
      2. Bare filename / relative path â†’ look up under
...
def _apply_subset(
    ...
    if cell_n != len(ids):
        raise ValueError(
            f"subset n={len(ids)} â‰  cell n={cell_n} â€” refusing to silently "
            "change scope. Fix the registry's `n` to match the subset file."
        )
    if "bench" in subset and subset["bench"] != cell["bench"]:
        raise ValueError(
            f"subset bench={subset['bench']!r} â‰  cell bench={cell['bench']!r}"
        )
...
def _get_gaia_scorer():
    ...
    ``BUJJI_GAIA_JUDGE_MODEL``); the judge backend is the ``cloud``
    engine, so ``BUJJI_CONFIG`` needs a ``[engine.cloud]`` section.
    """
    global _GAIA_SCORER
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                from bujji.evals_compat.bujji_direct import (
                    BujjiDirectBackend,
                )
                from bujji.evals_compat.gaia_exact import GAIAScorer

                judge_model = os.environ.get(
                    "BUJJI_GAIA_JUDGE_MODEL", "gpt-5-mini-2025-08-07"
                )
                try:
                    backend = BujjiDirectBackend(engine_key="cloud")
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    """GAIA scorer â€” normalized exact-match with an LLM-judge fallback.

    Uses the shared Bujji :class:`GAIAScorer`. The previous version
    ...
    """
    from bujji.evals_compat.types import EvalRecord
...
def _score_swebench(
    ...
    """
    from bujji.evals_compat.types import EvalRecord
    from bujji.evals_compat.swebench_harness import (
...
def _try_lock_exclusive(f) -> bool:
    """Attempt a non-blocking exclusive lock on *f*. Cross-platform.
...
def _cell_lock(out_dir: Path, cell_name: str):
    """Exclusive lock on ``<cell>/.lock`` to prevent concurrent runner stomps."""
    lock_path = out_dir / ".lock"
    f = lock_path.open("a+")
    if not _try_lock_exclusive(f):
        f.seek(0)
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import bujji.agents  # noqa: F401 â€” populate registry
    ...
    return agent_cls(
        engine=None,  # raw SDK calls â€” engine unused
...
def _run_one(
    ...
    *,
    ...
    ``TaskTimeout`` error row. The worker thread is then *abandoned* â€” a
    ...
    leak is bounded (one per timed-out task) â€” far cheaper than letting the
    ...
    def _target() -> None:
        ...
        except BaseException as e:  # noqa: BLE001 â€” never let the worker die silently
            box["row"] = _error_row(
                task, t0, f"{type(e).__name__}: {e}\n{traceback.format_exc()}"
            )
    ...
    if worker.is_alive():
        print(
            ...
            f"{task_timeout_s / 60:.1f}m â€” abandoning worker, recording error row",
            flush=True,
        )
        return _error_row(
            task,
...
def _heartbeat(done: int, total: int, row: Dict[str, Any], t_start: float) -> None:
    ...
    s = row.get("score") or {}
    sc = s.get("score")
    sc_str = f"{sc:.2f}" if isinstance(sc, (int, float)) else "â€”"
    print(
...
def _write_summary(
    ...
    # writing that as wall_time_s makes the cell look 20Ã— faster than it
    ...
    if n_processed == 0 and prior_wall > 0:
        wall = prior_wall
        # No work done this session â†’ keep prior energy total (don't add a
    ...
    summary = {
        ...
        # visible to the runner host. Cloud energy is **not** included â€”
...
def _run_cell_locked(
    ...
    *,
    ...
    if subset_path:
        ...
        print(f"[load] {cell['bench']} â†’ {len(tasks)} tasks", flush=True)
    ...
    # (env ``BUJJI_HYBRID_TASK_TIMEOUT_S``, 1800s) applies. 0 disables.
    mcfg = cell.get("method_cfg") or {}
    ...
    # returning â€” likely a daemon thread holding a non-Python lock that the
        ...
        while not watchdog_stop.wait(60.0):
            ...
            if time.time() - last_seen > watchdog_stale_s:
                print(
                    ...
                    f"(threshold {int(watchdog_stale_s)}s) â€” hard-exit, "
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m bujji.agents.hybrid.runner",
