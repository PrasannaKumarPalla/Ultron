...
When we run the hybrid paradigms (Minions, Advisors, Conductor, â€¦) at
...
(default ``max_retries=2`` on a small backoff) â€” under sustained pressure
...
either ``_base.py`` or ``minions._apply_patches_once`` is enough â€” the
...
2. Wraps ``chat.completions.create`` with:
   ...
   - A **per-org semaphore** (``BUJJI_OPENAI_MAX_CONCURRENCY``,
     default 4) that throttles sustained concurrency. Single bursts are
     fine â€” prepaid quotas wall on sustained rate, not on a brief spike.
   ...
   - On exhaustion, re-raises the last exception (it bubbles up to the
     ...
     ``results.jsonl`` â€” no silent drop).

Env knobs
---------

- ``BUJJI_OPENAI_MAX_CONCURRENCY`` (default ``4``) â€” semaphore
  capacity. Set to e.g. ``2`` if the wall is still hit; set to ``0`` to
  disable throttling entirely (passes through to the SDK).
- ``BUJJI_OPENAI_MAX_RETRIES`` (default ``8``) â€” outer retry loop
  cap (separate from the SDK's own ``max_retries``).
- ``BUJJI_OPENAI_RETRY_BASE`` (default ``2.0``) â€” base seconds for
  exponential backoff. Schedule is ``min(60, base * 2**attempt) * jitter``.
- ``BUJJI_OPENAI_RETRY_CAP`` (default ``60.0``) â€” max single-step
  sleep in seconds.
"""
...
_MAX_CONCURRENCY = _env_int("BUJJI_OPENAI_MAX_CONCURRENCY", 4)
_MAX_RETRIES = _env_int("BUJJI_OPENAI_MAX_RETRIES", 8)
_RETRY_BASE = _env_float("BUJJI_OPENAI_RETRY_BASE", 2.0)
_RETRY_CAP = _env_float("BUJJI_OPENAI_RETRY_CAP", 60.0)
...
def _sleep_for(attempt: int, exc: BaseException) -> float:
    ...
    # Full jitter â€” better tail behavior than equal jitter when many
...
def _wrap_create(orig: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapped(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        # the long retry loop (vLLM is mostly either up or down â€” a 60s
        # backoff just delays surfacing the failure). But brief
        # ConnectionError blips do happen mid-sweep (socket queue, brief
        # server warmup pause): give the local path a SHORT retry â€” 3
        # attempts, 1s/2s/4s â€” so we don't error an entire row on a
            ...
            try:
                ...
                try:
                    ...
                    print(
                        ...
                        f"{type(exc).__name__}: {str(exc)[:120]} â€” "
...
def patch_openai_globally() -> None:
    ...
    Safe to call from multiple modules â€” guarded by ``_PATCHED`` under a
    ...
    with _PATCH_LOCK:
        ...
        try:
            ...
            if cls is not None and not getattr(
                ...
                # Async wrapper is structurally different â€” only patch
