"""ArchonAgent â€” port of ScalingIntelligence/Archon.

Inference-time architecture search: layered (generator â†’ ranker â†’ fuser)
...
- **Local proposers** (generator layer): K samples from vLLM via an
  ...
  ``vllm_local`` model_type into Archon's ``GENERATE_MAP`` â€” that's the
...
- ``n_samples`` (int, default 5) â€” K proposers
- ``architecture`` (str, default ``"ensemble_rank_fuse"``)

  - ``"ensemble_rank_fuse"`` â†’ [K local generators, 1 cloud ranker, 1 cloud fuser]
  - ``"single_local"``       â†’ [1 local generator] (debug)
...
â€” either pip-install editable or set ``ARCHON_SRC`` to the checkout's
...
ARCHON_SWE_RANKER_SYS = (
    ...
    "Respond with a single integer on its own line â€” the index â€” "
...
def _stub_archon_imports() -> None:
    ...
    libraries aren't installed in the Bujji venv."""
...
_FUSER_FORMAT_REMINDER = (
    "\n\nIMPORTANT â€” output format: your synthesized response MUST honor the "
    ...
    "a unified diff inside a ```diff â€¦ ``` fence (SWE-bench), end with that "
    ...
    "format â€” the candidate responses may have done so, but your fused "
    "answer must not."
)
...
def _patch_archon_prompts() -> None:
    ...
    # Fuser imports the symbol by name at class-body time â€” rebind there too.
    from archon.completions.components import (
...
def _presets():
    return {
        ...
        # ``FINAL ANSWER:`` line â€” the scorer then had nothing to extract.
...
class ArchonAgent(LocalCloudAgent):
    """Layered (generator â†’ ranker â†’ fuser) inference-time search."""
    ...
    def _run_paradigm(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # way â€” they are infra failures, not model misses.
            raise
        finally:
            _set_anthropic_web_search(None)
    ...
    def _run_swe(
        ...
        # K independent runs, each on a FRESH workdir â†’ K diverse patches.
        ...
        # Parse â€” first integer on its own line.
        chosen_idx = 0
