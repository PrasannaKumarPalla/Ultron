"""AdvisorsAgent â€” inference-only port of advisor-models (Asawa et al., 2026).
...
``advisors-gaia-qwen9b-opus-30`` = 0.533, $0.02/task â€” within 3pp of
baseline-cloud at 30Ã— cheaper. The RL-trained variant would land higher.
...
Be concise (a short paragraph or a few bullet points). Do NOT restate the question. Do NOT provide a complete answer â€” only the feedback the model needs to improve its own next answer."""
...
class AdvisorsAgent(LocalCloudAgent):
    """Three-step executor â†” advisor â†” executor loop. See module docstring."""
        ...
        **kwargs: Any,
        ...
        if (
            ...
            raise ValueError(
                ...
                "web_search â€” otherwise search would silently no-op and the "
        ...
        # 1. Initial executor pass â€” advisor (Qwen) doesn't get tools;
    ...
    def _run_swe(
        ...
        # workdir â€” the advisor doesn't get a workdir, just the patch text.
        ...
        # 2. Advisor pass â€” local model critiques the produced patch
        local_model = _resolve_local_model(self._local_endpoint, self._local_model)
        advisor_prompt = ADVISOR_TEMPLATE.format(
            question=question,
            initial_response=(
        ...
        # 3. Final executor pass â€” FRESH workdir, advisor feedback folded
        # into the initial prompt. (Don't reuse initial workdir â€” that
