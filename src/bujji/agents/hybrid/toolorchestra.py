"""ToolOrchestraAgent â€” port of NVlabs ToolOrchestra (arXiv:2511.21689).
...
* ``"prompted"`` (default, legacy): a cloud model (Opus etc.) plays the
  ...
  a prompted upper-bound reference point â€” NOT the paper's setup.
...
* ``"rl"`` (paper-faithful): the RL-trained ``nvidia/Orchestrator-8B``
  ...
  vLLM's tool parser doesn't catch them) for three expert tools â€”
  ``enhance_reasoning``, ``answer``, ``search`` â€” exactly as in the
  upstream ``evaluation/tools.json``. Each tool's ``model`` arg
  (``answer-1``, ``reasoner-2``, ``search-3``, â€¦) is mapped to a real
  backend through ``EXPERT_MODEL_MAPPING`` â€” by default the frontier
...
2. It emits one ``tool_call`` per turn â€” ``search`` updates the
   context, ``enhance_reasoning`` appends code/exec output (we run the
   tool as a plain LLM call, no sandbox â€” the model just gets prose
...
from bujji.agents.hybrid._base import (
...
from bujji.agents.hybrid._prices import (
    PRICES,
    is_gpt5_family,
    supports_temperature,
)
from bujji.agents.hybrid.mini_swe_agent import (
    _clone_repo,
    _extract_diff,
    run_swe_agent_loop,
)
...
ORCHESTRATOR_SYS = """\
You are a tool-orchestrating agent. You coordinate a pool of workers to answer the user's question. Each turn you MUST emit exactly one JSON object â€” no prose, no markdown fences â€” taking one of two forms:
...
# edit the description text â€” Orchestrator-8B was RL-trained against this
...
# Map the orchestrator's `model` slot to a concrete Bujji worker spec.
...
def _expert_for(slot: str, local_model: Optional[str],
    ...
    """Map an upstream model slot (`answer-1`, `search-3`, â€¦) to a worker spec.
    ...
    # Fallback if no local â€” gpt-5-mini.
    return {
...
#                    (paper uses Qwen-2.5-Math-72B; not on OpenRouter â€” see doc)
#   answer-math-2 -> Qwen-2.5-Coder-32B via OpenRouter
#                    (paper uses Qwen-2.5-Math-7B; not on OpenRouter â€” see doc)
#   search-*   -> Tavily search (paper)
#
# `enhance_reasoning` is dispatched through the coder specialist regardless of
# slot tier â€” the orchestrator emits one of `reasoner-{1,2,3}` and the caller
...
def _paper_expert_for(
    ...
    if slot.endswith("-1"):
        # Tier-1 frontier reasoner / answer â€” paper uses GPT-5.
        return {
            "name": f"frontier:{slot}",
            "type": "openai",
            "model": "gpt-5",
...
def _call_tavily_search(
    ...
    Token counts are reported as zero (no LLM was billed); the Bujji
...
_MODAL_APP_NAME = "bujji-toolorchestra-sandbox"
...
def _call_modal_python(code: str, timeout_s: int = 60) -> Tuple[str, int]:
    ...
    the returned string with a non-zero rc â€” we never raise back to the
    ...
    try:
        ...
        # Python image too. We rely on stdlib only â€” no extra pip installs.
...
def _call_orchestrator_with_tool_calls(
    ...
    *,
    ...
    into the SDK field while leaving ``content == ''`` â€” and the text-tag
...
def _paper_pool(
    ...
    worker_id, so this list is purely informational â€” `_paper_expert_for`
...
def _parse_rl_tool_call(content: str, sdk_tool_calls: Any) -> Optional[Dict[str, Any]]:
    ...
    content. We take the first tool call only â€” Orchestrator-8B was trained
...
def _build_user_prompt(
    ...
    pieces.append(
        "Emit the next JSON action object now â€” exactly one object, no prose."
    )
    return "\n\n".join(pieces)
...
def _default_pool(
    ...
    gemini-2.5-pro, â€¦) route their frontier slot to the right SDK.
    """
    ep = (cloud_endpoint or "anthropic").lower()
    if ep not in ("anthropic", "openai", "gemini"):
        ep = "anthropic"
    ...
    pool.append({
        ...
        "description": (
            ...
            "code review, or a final synthesis pass. Expensive â€” use sparingly."
        ),
    })
    pool.append({
        "id": len(pool),
...
#
# Paper-match additions (2026-05-19) â€” opt in via `method_cfg.pool = "paper"`:
#   `tavily-search`  â€” Tavily API search (the paper's web tool).
#   `openrouter`     â€” OpenAI-compatible client at openrouter.ai/api/v1.
#                      Used for the code/math specialists and Llama-3.3-70B /
#                      Qwen3-32B generalists.
#   `modal-python`   â€” One-shot Python exec in a fresh Modal Sandbox (the
...
def _resolve_worker_pool(
    ...
    ``model`` â€” it defaults to ``claude-haiku-4-5``. OpenAI and Gemini
...
def _call_worker(
    ...
    if wtype == "modal-python":
        ...
        # charges per sandbox-second â€” not tracked here.
...
def _swe_call_worker(
    ...
    if wtype == "vllm":
        ...
        # Unknown type â€” one-shot fallback.
        text, p, c, is_local, extra, n_searches = _call_worker(worker, prompt, cfg)
        return text, p, c, is_local, extra, n_searches, 0
...
class ToolOrchestraAgent(LocalCloudAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Validate `method_cfg.worker_pool` early â€” surfaces config errors
    ...
    def _run_prompted(
        ...
        **kwargs: Any,
        ...
        #   1. `cfg["workers"]` â€” legacy direct override, used by tests.
        #   2. `cfg["worker_pool"]` â€” cell-config override; validated +
        #      $local/$cloud substituted.
        #   3. `_default_pool(...)` â€” heterogeneous default.
        if cfg.get("workers"):
            workers = cfg["workers"]
        else:
        ...
        try:
            ...
            # produce text only â€” calling a worker is one tool call's worth
            ...
            for turn in range(1, max_turns + 1):
                ...
                if kind == "call_worker":
                    ...
                    continue
                # Unknown action kind â€” treat as parse failure.
                parse_failures += 1
            ...
            if final_answer is None:
                ...
                # Search workers are excluded â€” they answer fact-lookup
            ...
            # In SWE mode, the authoritative output is the working-tree diff â€”
    ...
    def _run_rl(
        ...
        **kwargs: Any,
        ...
        # ``context_str`` mirrors the upstream's running context â€” accumulates
        ...
        try:
            for turn in range(1, max_turns + 1):
                ...
                # get the SDK-level ``tool_calls`` object back â€” `_call_vllm`
                ...
                # non-opus-gaia cell â€” see docs/reports/toolorchestra.md). Both
                ...
                if not valid:
                    ...
                    context_str += (
                        ...
                        f"name={name!r} slot={slot!r} â€” try again.]\n"
                    )
                    continue
                ...
                # Dispatch â€” the orchestrator only conveys a tool/model
                ...
                if name == "search":
                    if paper_mode:
                        ...
                        # schema but useful) â€” prefer it; else fall back to
                        # the raw question.
                        q = args.get("query")
                    ...
                    if paper_mode:
                        w_input = (
                            ...
                            "must run with the Python stdlib only â€” no extra pip "
                            "installs."
                        )
                    else:
                        w_input = (
                            ...
                            "answer â€” the orchestrator will collect your reasoning and "
            ...
            meta = {
                ...
                "traces": {
                    ...
                    "note": (
                        ...
                        "our hybrid worker types â€” see toolorchestra.py docstring."
                    ),
                },
            }
            return final_answer, meta
