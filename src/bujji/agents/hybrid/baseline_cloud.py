"""BaselineCloudAgent â€” cloud-only reference for the hybrid ablation.

Used as the "what does the cloud do alone?" row in the n=100 ablation
matrix (see ``.bujji/experiments/hybrid/docs/results-table.md``).
No local model is involved â€” ``local_*`` settings are ignored.
...
so the model gets to run bash and read the repo â€” same wiring as the
...
from bujji.agents.hybrid._prices import cost as estimate_cost
from bujji.agents.hybrid._prices import default_max_output_tokens
...
class BaselineCloudAgent(LocalCloudAgent):
    ...
    - ``cloud_max_tokens`` (int, default 4096 / 16384 for reasoning models):
      ...
      answer text; at 4096 they silently truncated 18â€“26% of GAIA cells with
    ...
    def _run_paradigm(
        ...
        **kwargs: Any,
        ...
        # One-shot direct cloud call. GAIA only â€” SWE goes through the
