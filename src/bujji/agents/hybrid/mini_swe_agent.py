"""MiniSWEAgent â€” vendored, ~330-line port of mini-SWE-agent v2.

Single-LLM agent loop with a ``bash`` tool, run inside a per-task git
clone. The model iterates: read files, grep, run tests, edit, retry â€”
...
1. **Standalone agent** â€” :class:`MiniSWEAgent` registered as
   ``mini_swe_agent``. Use it directly as the agent for a cell.
2. **As a worker subroutine inside another paradigm** â€” call
...
- No Docker sandbox. We clone the SWE-bench repo into a tempdir and
  ...
  untrusted â€” model can run ``rm -rf`` against its own workdir, but the
  workdir is disposable. Don't run this on a host with secrets in the
  CWD.
- One tool, ``bash``. No separate ``submit`` â€” the loop ends when the
...
# OpenAI/Anthropic JSON-Schema lower-case form is silently dropped â€” the
...
You do NOT need to `cd` anywhere â€” just run `ls`, `cat path/to/file`, etc. \
relative to the repo root.
- Each `bash` call is a fresh shell â€” there's no persistent cwd, env, or \
shell state carried between calls (but cwd is reset to the repo root each \
call, so this is fine for normal exploration).
- Don't run `git commit`, `git stash`, or anything that mutates git state â€” \
...
BASH_TOOL_ANTHROPIC = {
    ...
    "description": (
        ...
        "and the exit code. Each call is a fresh shell â€” no persistent state."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
# prefix commands with ``cd /testbed`` â€” the standard container repo path.
...
# ``&&``, aborts the whole command â€” so the agent burns every turn and
...
def _rewrite_testbed_paths(command: str, workdir: Path) -> str:
    ...
    - A leading ``cd /testbed && ...`` (or ``;``) is stripped â€” bash already
    ...
    # Bare ``cd /testbed`` with nothing after it â†’ no-op into the workdir.
...
def _decode_bash_output(raw: bytes, exit_code: int) -> str:
    ...
    2. If the result looks predominantly binary â€” contains a NUL byte OR
       has more than ~5% U+FFFD replacement chars after decode â€” replace
       ...
       (no NUL byte â‡’ probably text-ish; LLMs handle the occasional
       replacement char fine).
    """
    if not raw:
        return ""
    decoded = raw.decode("utf-8", errors="replace")
    if b"\x00" in raw or decoded.count("ï¿½") * 20 > len(decoded):
...
def _run_bash(
    ...
    draining the pipe â€” which a surviving grandchild holds open forever,
    ...
    if len(stdout) > output_cap:
        stdout = stdout[:output_cap] + f"\nâ€¦[+{len(stdout) - output_cap} chars truncated]"
        truncated = True
    if len(stderr) > output_cap:
        stderr = stderr[:output_cap] + f"\nâ€¦[+{len(stderr) - output_cap} chars truncated]"
        truncated = True
    return {
        "stdout": stdout,
        "stderr": stderr,
...
def _extract_diff(workdir: Path) -> str:
    """``git diff`` against the base commit â€” the final SWE-bench patch."""
    proc = subprocess.run(
        ["git", "diff", "--no-color"],
...
# ---------- Cloud loop (dispatcher â†’ per-endpoint multi-turn tool loops) ----------
...
def _loop_cloud_openai(
    ...
    *,
    ...
    for turn in range(1, max_turns + 1):
        ...
        # message gets replayed. Use ``""`` â€” explicitly allowed by the
        ...
        if not tool_calls:
            ...
            # is empty/short â€” treating that as "model done" exits the loop
            ...
            if (
                ...
                continue
            # No tool call â†’ the model is done. Same termination rule as
...
def _loop_cloud_gemini(
    ...
    *,
    ...
    - Parameters must be a Schema-shaped dict with capitalized type names
      ...
      dropped â€” the model never produces a call.
    ...
    for turn in range(1, max_turns + 1):
        ...
        if cand_content is not None:
            ...
            # Safety-filtered / empty â€” synthesize an empty model turn so
        ...
        if not function_calls:
            ...
            #   MALFORMED_FUNCTION_CALL â€” model wanted to call bash but
            #     produced unparseable args (24/100 of broken n=100 SWE
            #     cells, 2026-05-15). Empty text + empty function_calls â†’
            #     loop would exit with no final summary.
            #   MAX_TOKENS â€” same shape as OpenAI's ``length``; truncated
...
_COMPACT_PROMPT = (
    ...
    "and the current plan. Be terse â€” no preamble, no quoted output, just facts."
)
...
def _compact_local_messages(
    ...
    *,
    ...
    synthetic = {
        "role": "system",
        "content": f"[turns 1â€“{n_turns_folded} elided: {summary}]",
    }
    ...
    *,
    ...
    for turn in range(1, max_turns + 1):
        ...
        try:
            ...
            # failure â€” the runner records the row as errored.
            msg = str(exc)
            is_ctx = (
