"""ConductorAgent â€” static-DAG planner (Sakana AI, arXiv 2512.04388).
...
1. **Plan** â€” the conductor reads the question + numbered worker pool
   and emits three lists ``(model_id, subtasks, access_list)`` in JSON,
   up to 5 steps.
2. **Execute** â€” for each step ``i``: build the worker prompt from
...
acc / $0.22 per task â€” +10pp vs baseline-cloud at ~15Ã— cheaper.
...
CONDUCTOR_STRICTER = (
    ...
    "Reply with ONLY a single JSON object â€” no prose, no code fences, no commentary "
    "â€” containing exactly the three keys model_id (list[int]), subtasks (list[str]), "
...
def _default_pool(local_model: Optional[str], local_endpoint: Optional[str]) -> List[Dict[str, Any]]:
    """Default worker pool â€” faithful to the Sakana Conductor paper (arXiv 2512.04388).
    ...
    worker is in the paper's default â€” cells that want one should supply it
    ...
    every provider is disabled the result is the empty list â€” the caller's
    ...
    if not os.environ.get("OJ_CONDUCTOR_DISABLE_OPENROUTER"):
        ...
        pool.append({
            ...
            "description": (
                ...
                "summarization, or formatting â€” does NOT spend tokens on "
                "internal reasoning."
            ),
        })
        pool.append({
...
# Web-search is NOT supported here â€” toolorchestra has the web-search
# dispatcher. OpenRouter is OpenAI-compatible; Gemini is text-only (no
# tool-call parity with Anthropic â€” see `_base._call_gemini`). Both are
...
def _resolve_worker_pool(
    ...
    ``vllm`` / ``openai`` / ``anthropic`` / ``openrouter`` / ``gemini`` â€”
    ...
    for raw in override:
        ...
        if endpoint == "vllm":
            if not entry.get("base_url"):
                # Default to the local endpoint if not specified â€” matches
            ...
            # OpenRouter is exempt â€” its model space is huge and varies
    ...
    # but execution dispatches via `workers[model_id]` â€” list *position*. If a
...
def _build_conductor_prompt(
    ...
    *,
    ...
    if capable:
        ...
        # No search-capable worker at all â€” the run-level guard raises
...
def _swe_worker_step(
    ...
    if ep == "vllm":
        ...
        # OpenAI-via-vllm-shaped only). Fall back to one-shot for those â€”
...
class ConductorAgent(LocalCloudAgent):
    ...
    def _run_paradigm(
        ...
        **kwargs: Any,
        ...
        #   3. `_default_pool(...)` â€” heterogeneous default (Opus +
        ...
        # Determine swe_mode up front â€” needed so the planner prompt can
        ...
        # 1. Plan â€” when web_search is on (GAIA), the prompt names which
        ...
        # planner constraint above) â€” reuse them.
        ...
        # loop in `_call_worker` and ground their answers. GAIA-only â€” SWE
        # workers use bash not web_search. openrouter / vllm workers can't
        # ground and ignore the flag. If the worker pool has NO
        # search-capable worker, web_search can never run on any step â€”
        ...
        if ws_enabled and search_backend != "tavily" and not swe_mode:
            ...
            if not search_workers:
                ...
                raise ValueError(
                    ...
                    "Add one of those to the pool or disable web_search â€” "
        ...
        try:
            ...
            for i, (mid, subtask, access) in enumerate(
                ...
                # record a warning into the trace (don't crash â€” the step
            ...
            # the working tree at the end â€” replace whatever the last
