"""Load system prompt and few-shot overrides from $BUJJI_HOME.
...
- System prompts: ``$BUJJI_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$BUJJI_HOME/agents/{name}/few_shot.json``
...
def _bujji_home() -> Path:
    """Resolve the Bujji root, honoring BUJJI_HOME / XDG_DATA_HOME."""
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    Looks for ``$BUJJI_HOME/agents/<agent_name>/system_prompt.md``.
    ``BUJJI_HOME`` defaults to ``~/.bujji`` when unset.
    """
    home = _bujji_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$BUJJI_HOME/agents/<agent_name>/few_shot.json``.
