...
# Rolling summary fed back into the next tick's prompt, so it stays bounded â€”
...
class AgentManager:
    ...
    def __init__(self, db_path: str, *, clear_stale_running: bool = False) -> None:
        ...
        # owns the scheduler) may sweep runningâ†’idle on boot. Short-lived CLI
        # commands (`bujji agents list/info/...`) and the SystemBuilder path
        ...
        # running agent back to "idle" â€” which is exactly why `list` reported
    ...
    def _clear_stale_running_state(self) -> None:
        ...
        Tick worker threads are ``daemon=True`` â€” when the server process
    ...
    def create_agent(
        ...
        from bujji.agents.executor import _AGENT_TICK_DEFAULT_MODEL
    ...
    def start_tick(self, agent_id: str) -> None:
        ...
        overtaken rather than refused â€” otherwise a crash with no server
    ...
    def list_templates() -> List[Dict[str, Any]]:
        ...
        # Built-in templates
        try:
            tpl_dir = importlib.resources.files("bujji.agents") / "templates"
