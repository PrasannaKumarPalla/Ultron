# ruff: noqa: E501
"""Proactive Agent â€” runs on a cron (default 5am local) to autonomously handle
...
4. For each proposed action:
   - TRIVIAL tier          â†’ queue + immediately approve
   - Known always_approve  â†’ queue + immediately approve
   - Known always_deny     â†’ skip silently
   - Everything else       â†’ queue as pending, notify user
...
called from your app startup:
    ...
    from bujji.agents.proactive_agent import register_cron
...
from bujji.agents._stubs import AgentContext, AgentResult, ToolUsingAgent
...
from bujji.tools.approval_store import (
...
from bujji.tools.proactive_tools import get_store
...
Each action object must have these fields:
  ...
  - payload: dict with the data needed to execute. ALWAYS include:
      - doc_id: copy the value of ``id=...`` from the digest line, EXACTLY
        ...
        ids like ``"gmail:wells_fargo"`` or ``"msg_1"`` â€” the executor
...
Tier guidance:
  trivial â€” read-only or categorization only, no external effect
  low     â€” reversible, routine (delete a known-spam sender, archive newsletter)
  medium  â€” affects another party but is expected (reply to a simple scheduling text)
  high    â€” sends a message in the user's voice for the first time, or irreversible
...
HARD LIMITS â€” these keep responses parseable:
  - Output AT MOST 8 action objects.  Pick the highest-value ones (most
    clearly safe-to-delete or obviously useful to handle).
  - Keep each `reasoning` field to ONE short sentence (â‰¤ 15 words).
  - Keep each `description` field to ONE short sentence (â‰¤ 15 words).
  - No nested objects beyond what the schema requires.
  - Your entire visible response MUST be ONLY the fenced JSON block â€”
...
[
  {
    ...
    "tier": "low",
    "reasoning": "Routine newsletter â€” safe to archive."
  },
  {
...
calendar duplicates â€” these are the items the user wants triaged.

User context is provided below â€” use it to tailor decisions to their patterns.
"""
...
def _build_notification_channel(channel_spec: str) -> Optional[Any]:
    ...
    Supports:
      ``imessage:+15551234567``  â€” sends via AppleScript directly
      ``telegram:123456789``     â€” instantiates TelegramChannel
      ``slack:D0123456789``      â€” instantiates registered Slack channel
    ...
    if channel_type == "imessage":
        from bujji.channels._stubs import (
            BaseChannel,
            ChannelStatus,
        )
        ...
        class _IMessageShim(BaseChannel):
            ...
            def send(
                ...
                from bujji.channels.imessage_daemon import send_imessage
    ...
    try:
        import bujji.channels  # noqa: F401  trigger registration
        from bujji.core.registry import ChannelRegistry
...
class ProactiveAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        from bujji.tools.channel_tools import ChannelSendTool
        from bujji.tools.digest_collect import DigestCollectTool
        from bujji.tools.proactive_tools import (
            CheckPermissionTool,
        ...
        # The agent emits a JSON array of proposals â€” one entry per actionable
        # item â€” and a typical morning digest produces dozens. The default
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        # --- Step 1: Collect data â€” only items user hasn't acted on ---
        ...
        if not collect_result.success or not collect_result.content.strip():
            ...
            return AgentResult(
                content="No data collected from connectors â€” nothing to do.",
                turns=1,
            )
        ...
        # Debug log â€” write the raw LLM output and what we parsed out so a
    ...
    def _build_notification(
        ...
        if executed:
            ...
            for r in successes:
                lines.append(f"  âœ“ {r['description']}")
            for r in failures:
                lines.append(f"  âœ— {r['description']} â€” {r.get('message', 'error')}")
