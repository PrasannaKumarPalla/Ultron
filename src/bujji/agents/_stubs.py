...
class ExecutionMode(str, Enum):
    """How an agent is driven at runtime.

    - ``ON_DEMAND``: synchronous request/response via ``QueryOrchestrator``
      (the default for every agent).
    - ``SCHEDULED``: driven on an interval/cron by ``operators.manager``.
    - ``CONTINUOUS``: long-lived managed agent driven by ``agents.manager``.

    Historically this was implied by *which subsystem* called ``run()``.
    Making it a first-class attribute lets the registry list agents by mode
    and lets the orchestrator reject dispatching a non-on-demand agent
...
from bujji.core.types import Conversation, Message, Role, ToolResult
...
class BaseAgent(ABC):
    ...
    #: How this agent is driven at runtime. Defaults to on-demand so every
    #: existing agent keeps its current (synchronous) dispatch behavior.
    ...
    def _apply_persona(self, system_prompt: Optional[str]) -> Optional[str]:
        ...
        ``bujji ask`` (#376) by *appending* persona to â€” never replacing â€”
    ...
    def _build_messages(
        ...
        *,
        ...
        # Inject memory facts from context into the system prompt
        if context and context.memory_results:
            facts_text = "\n".join(
                f"- {r}" if isinstance(r, str) else f"- {getattr(r, 'text', str(r))}"
                for r in context.memory_results
            )
            memory_block = f"What you know about the user:\n{facts_text}"
...
class ToolUsingAgent(BaseAgent):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            from bujji.agents.loop_guard import LoopGuard, LoopGuardConfig
