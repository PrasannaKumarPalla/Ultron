...
from bujji.core.types import Message, Role, ToolCall, ToolResult
...
class OrchestratorAgent(ToolUsingAgent):
    ...
    def _run_structured(
        ...
        **kwargs: Any,
        ...
        if self._system_prompt:
            ...
            from bujji.agents.prompt_registry import (
                build_system_prompt,
            )
    ...
    async def arun(
        ...
        **kwargs: Any,
        ...
        """Async wrapper: runs ``run()`` in an executor so it doesn't block the event loop."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, lambda: self.run(input, context, **kwargs))
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        """Yield the final response token-by-token after completing the tool loop.

        The tool-calling loop itself is not streamed (local models often don't
        support mid-tool streaming).  Once the loop finishes the text is chunked
        by word so the UI gets progressive display without a blocking wait.
        """
        result = await self.arun(input, context, **kwargs)
