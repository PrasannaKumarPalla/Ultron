...
from bujji.agents.prompt_loader import (
    load_few_shot_exemplars,
    load_system_prompt_override,
)
from bujji.agents.rlm_repl import RLMRepl
...
from bujji.tools._stubs import BaseTool, build_tool_descriptions
...
RLM_SYSTEM_PROMPT = (
    ...
    "- `tool_call(tool_name: str, args: dict) -> str` â€” Execute an "
    "Bujji tool and return its textual output.\n"
    "- `read_file(path: str, max_lines: int = 120) -> str` â€” Read the "
    "first N lines of a file through the real file_read tool.\n"
    "- `read_file_chunk(path: str, start_line: int, end_line: int) -> str` "
    "â€” Read only a bounded line range from a file.\n"
...
class RLMAgent(ToolUsingAgent):
    ...
    def _execute_tool_from_repl(self, tool_name: str, params: Dict[str, Any]) -> str:
        """Execute a real Bujji tool from within the REPL."""
