...
from bujji.core.types import Message, Role, ToolCall, ToolResult, _message_to_dict
...
1. TOOLS: You have access to tools via native function calling. The list
   below shows what is available â€” invoke them through the function-calling
...
- Prefer substantive tools (web_search, memory_retrieve) over reasoning-only
  tools (think) â€” `think` does not gather new information, only reorganises
...
class MonitorOperativeAgent(ToolUsingAgent):
    ...
    accepts_tools = True
    #: Long-lived managed agent driven by agents.manager / AgentExecutor, not by
    #: synchronous `bujji ask`.
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        # Honor SOUL.md / MEMORY.md / USER.md persona files like `bujji ask`,
