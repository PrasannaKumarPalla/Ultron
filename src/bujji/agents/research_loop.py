...
from bujji.connectors.hybrid_search import HybridSearch, SearchHit
from bujji.core.types import Message, Role, ToolCall
from bujji.engine._base import InferenceEngine
            ...
            "properties": {
                ...
                "person": {
                    ...
                    "description": (
                        ...
                        "substring of the name or email address â€” 'Kelly' or "
                ...
                "sources": {
                    ...
                    "description": (
                        ...
                        "Granola notes\" â†’ ['granola']; \"check Slack and Gmail\" "
                        "â†’ ['slack', 'gmail']). Valid IDs include: gmail, slack, "
...
Strategy:
  ...
  2. When the user mentions ANY time window â€” "this past week", "recently", "last month", "past few days", "yesterday" â€” you MUST translate it to a `time_range` parameter. Today is {today}.
  ...
  4. When the user names a specific data source â€” "my Granola notes", "in Slack", "from my email" â€” you MUST pass `sources=[...]` with the matching connector ID. Only use IDs that appear in the connected-sources list above; do NOT invent or assume sources that are not connected. Common synonyms: "meeting notes"/"meetings"/"transcripts" â†’ granola; "email"/"inbox" â†’ gmail; "DMs"/"channels" â†’ slack. Without this filter the search returns mail/messages ABOUT a tool instead of records FROM that tool.
  4a. Never apologize about sources that aren't in the connected-sources list â€” if the user asks about "Notion" but Notion isn't connected, just say "Notion isn't connected, but here's what I found in {available_sources}" and answer from what is available.
  5. If the first structured search returns nothing useful, broaden with a semantic query and drop filters one at a time.
  6. You have a clarify tool. Only use it AFTER at least one search attempt. Use it when: you found multiple ambiguous matches (e.g. 3 different people named John), search returned zero results and the query might need reframing, or the scope is too broad to synthesize meaningfully. Never use clarify before searching â€” always try first.
  7. After receiving a clarify response, use the information to construct a precise search with the correct person, time_range, sources, and query parameters. Never send an empty query or a query with no parameters â€” extract every concrete signal from the user's reply (names, dates, topics, sources) and put it on the call.
  8. Tool calls â€” search AND clarify â€” share a budget of 5 total. Spend wisely.

Synthesis rules:
  - Cite sources as individual numbers in square brackets. Always separate â€” write [4] [7] [20], never [4, 7, 20]. Never format citations as markdown links. Just the number in brackets: [1]. The `ref` field on each hit is the citation number.
  - Quote sender / date / subject when relevant â€” the user wants attribution.
...
def _trim_thread_context(ctx: List[Dict[str, Any]], cap: int) -> List[Dict[str, Any]]:
    ...
    trimmed.append({"snippet": f"â€¦ {len(ctx) - cap} more chunks in thread â€¦"})
    return trimmed
...
def _hit_url(source: str, document_id: str) -> str:
    ...
    sources whose doc_id encodes everything the permalink needs â€” Gmail
    ...
    - **Hex message id** (``19dfa2ccbeff78b0``) â€” what the OAuth Gmail
      connector stores. Resolves directly via ``#all/<id>`` permalink.
    - **RFC822 Message-ID** (``<CABCD@mail.gmail.com>``) â€” what the IMAP
...
def renumber_citations(
    ...
    (search 1 returns 1..20, search 2 returns 21..40, â€¦). When the
    ...
    from the returned ``sources`` list â€” only the ones the user can
    ...
    for m in _CITE_RE.finditer(text):
        ...
        if src is None:
            # Synthesis cited a ref that doesn't exist in the corpus â€”
...
def build_sources_for_client(
    ...
    *,
    ...
    One entry per hit, in the same order the planner sees them â€” so a
    ...
    for i, h in enumerate(hits[:total_cap]):
        ...
        # ``web_url``, Notion's page URL, etc.) â€” it's the only reliable
...
def _default_clarify_handler(question: str) -> str:
    ...
    print(f"\033[1mðŸ¤” Clarification needed:\033[0m {question}", file=sys.stderr)
    try:
        answer = input("> ").strip()
    except EOFError:
...
class ResearchAgent:
    ...
    on_event:
        ...
        Receives a dict in one of these shapes:
          - ``{"type": "search_call", "arguments": {...}}`` â€” about to call search
          - ``{"type": "search_result", "num_hits": N, "top_titles": [...], "sources": [{"ref": 1, "title": ..., "sender": ..., "date": ..., "source_id": ..., "url": ...}, ...]}`` â€” search returned
          - ``{"type": "clarify_call", "question": "..."}`` â€” about to ask for clarification
          - ``{"type": "clarify_response", "response": "..."}`` â€” clarification received
          - ``{"type": "final_answer", "text": "..."}`` â€” synthesis ready
    ...
    def _emit(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug("on_event callback raised %s â€” ignoring", exc)
    ...
    def _execute_search(self, args: Dict[str, Any]) -> ToolInvocation:
        ...
        titles = [h.title or (h.content_snippet[:60] + "â€¦") for h in hits[:5]]
    ...
    def _execute_clarify(self, args: Dict[str, Any]) -> ToolInvocation:
        ...
        if not question:
            return ToolInvocation(
                tool_name="clarify",
                arguments={"question": ""},
                response="(no question provided by agent â€” skipping clarify)",
            )
    ...
    def _resolve_available_sources(self) -> List[str]:
        ...
        ``[]`` so the prompt still formats â€” better empty than crashing.
        """
    ...
    def run(self, query: str) -> ResearchResult:
        ...
        if sources_list:
            ...
            sources_blurb = (
                "(no connected sources â€” tell the user to connect a "
        ...
        for _ in range(self._max_iterations + 1):
            ...
            if not tool_calls_raw:
                ...
                # Empty content with no tool call â€” push a synthesis prod
                if invocations:
            ...
            for tc in tool_calls_raw:
                ...
                if name == "search":
                    ...
                    # search has run â€” silently accept; the rule lives in the
        ...
        # an answer â€” bailing out with a sentinel string is never useful to
        ...
        messages.append(
            Message(
                ...
                content=(
                    ...
                    "request more tool calls â€” write the final answer as "
        ...
        if not answer:
            answer = (
                "(no synthesis available â€” the search budget was exhausted "
