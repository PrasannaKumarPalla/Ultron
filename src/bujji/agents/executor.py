...
from bujji.agents._stubs import AgentResult
from bujji.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
from bujji.core.events import EventBus, EventType
...
# action tools â€” producing tickless prose from training-data memory.
...
class AgentExecutor:
    ...
    Constructor receives a BujjiSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection â€” called after BujjiSystem is constructed."""
        self._system = system
    ...
    def _build_routing_constraints(self):
        """Build a ConstraintChecker from config + telemetry, or None.

        Returns None when no ceiling is configured, telemetry is unavailable,
        or anything goes wrong — routing must never break because the optional
        constraint gate could not be assembled.
        """
        if self._system is None:
            return None
        try:
            from bujji.learning.routing.constraints import (
                ConstraintChecker,
                RoutingConstraints,
            ...
            constraints = RoutingConstraints.from_config(config.learning.routing)
            if not constraints.enforced:
                return None
            agg = TelemetryAggregator(config.telemetry.db_path)
            try:
                stats = stats_by_model_from_aggregator(agg)
            finally:
                agg.close()
            return ConstraintChecker(
                constraints, stats, bus=getattr(self._system, "bus", None)
            )
        except Exception:
            return None
    ...
    def execute_tick(
        ...
        guard â€” bailing out with no end_tick(), leaving the agent stuck in
        ...
        try:
            ...
            tick_duration = max(time.perf_counter() - tick_timer, 1e-9)
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from bujji.agents import AgentRegistry
        ...
        # Resolve engine + model from BujjiSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in BujjiSystem")
        model = (
            config.get("model")
        ...
        if router_policy_key and self._system:
            try:
                from bujji.core.registry import RouterPolicyRegistry
                from bujji.learning.routing.router import (
                    build_routing_context,
                )

                create_kwargs: dict[str, Any] = {"available_models": [model]}
                # Attach first-class cost/latency/energy constraints to the
                # heuristic policy when telemetry + a ceiling are available, so
                # an over-budget model is dropped (and the hit logged/emitted).
                checker = self._build_routing_constraints()
                if checker is not None and router_policy_key == "heuristic":
                    create_kwargs["constraints"] = checker
        ...
        # TOOL_CALL_START/END â€” without this, ToolExecutor's ``self._bus``
        ...
        # agent's own ToolExecutor. Set by CLI paths like `bujji agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `bujji ask` path so they no
        ...
        logger.info(
            "Agent %s: tool wiring â€” %d tools resolved (%s), agent class %s",
            agent["name"],
            len(tool_instances),
        ...
        # in as "Previous context" â€” that caused the model to parrot its
        ...
        if (
            ...
            try:
                from bujji.tools.storage.context import (
                    ContextConfig,
                    format_context,
                )
    ...
    def _save_trace(
        ...
        from bujji.core.types import StepType, Trace, TraceStep
