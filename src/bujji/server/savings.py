...
# `bujji.core.types` so the telemetry layer can read it without
# crossing the server â†’ telemetry layering. Re-exported here for
# backward compatibility with existing imports of `from
# bujji.server.savings import TOKEN_COUNTING_VERSION`.
from bujji.core.types import TOKEN_COUNTING_VERSION  # noqa: E402,F401
...
def compute_savings(
    ...
    by summing each turn's full prompt â€” which counts the system prompt
    N times for an N-turn conversation â€” so the fallback turned the FLOPs
    and energy estimates into NÃ—-too-high numbers. That was the dominant
    ...
    missing, treat `prompt_tokens_evaluated` as 0 â€” so the FLOPs/energy
    ...
    rows show `â€”` instead of a misleading zero.
    """
