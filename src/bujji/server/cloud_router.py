...
~/.bujji/cloud-keys.env fallback for non-desktop/manual setups. Uses
...
def get_provider(model: str) -> str | None:
    ...
    # Ollama pulls HuggingFace GGUF models under an "hf.co/..." (or
    # "huggingface.co/...") id — these run locally via Ollama, not a cloud
    # provider, so they must NOT be filtered out of the local model list
    # (e.g. the sarvam models: hf.co/bartowski/sarvam-1-GGUF:...).
    if model.startswith(("hf.co/", "huggingface.co/")):
        return None
...
async def _stream_anthropic(
    ...
    if not api_key:
        raise ValueError("ANTHROPIC_API_KEY not set â€” add it in the Cloud Models tab")
