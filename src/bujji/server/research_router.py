"""HTTP route: ``POST /api/research`` â€” agentic research over the knowledge store.

Drives :class:`bujji.agents.research_loop.ResearchAgent` and streams a
custom SSE event schema back to the client:

* ``search_call``     â€” about to invoke ``HybridSearch.search`` (with arguments)
* ``search_result``   â€” search returned (num_hits, top_titles)
* ``synthesis``       â€” final answer, emitted in word-window chunks for an
...
* ``done``            â€” sentinel marking the end of the stream

Clarify is **disabled for the web session** â€” the agent's clarify_handler is
...
from bujji.agents.research_loop import (
...
from bujji.core.types import TelemetryRecord
...
def _record_research_telemetry(
    *,
    ...
    rolls research into the same numbers â€” Power (W) and Energy (kJ) stop
    reading 0.0 after a research-only session.

    Failures are swallowed â€” telemetry persistence is best-effort and must
...
# ---------------------------------------------------------------------------
# Live GPU sampler â€” streams power/energy events during agent.run()
# ---------------------------------------------------------------------------


class _LiveGPUSampler:
    """Background pynvml poller â€” emits live power and accumulated energy.
    ...
    False and ``start()`` / ``stop()`` are no-ops â€” research still runs, the
...
class ResearchRequest(BaseModel):
    ...
    # ignore it â€” the planner always runs on DEFAULT_PLANNER_MODEL.
...
async def _stream_research(query: str, model: str) -> AsyncGenerator[str, None]:
    ...
    Three error envelopes â€” setup, worker, consumer â€” all funnel into the
    ...
    # yield error + done and return â€” nothing has been emitted yet so the
    ...
    def _emit_live_sample(power_w: float, energy_j: float, duration_s: float) -> None:
        # Called from the sampler's worker thread â€” bounce onto the asyncio
    ...
    def _run() -> None:
        ...
        try:
            ...
            # rolls research into the same Power/Energy numbers as chat â€”
            ...
            # to the terminal `done` frame. Internal event type â€” never sent to
            # the client directly.
            loop.call_soon_threadsafe(
                queue.put_nowait,
    ...
    try:
        while True:
            ...
            # Internal usage marker â€” capture for the done frame, don't forward.
            if etype == "_usage":
        ...
        # The worker may still be cleaning up (rarely) â€” make sure we don't
