...
logger = logging.getLogger("bujji.server.agent_manager")
...
class _LightweightSystem:
    ...
    full BujjiSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    """
    try:
        from bujji.engine._discovery import get_engine
        ...
        try:
            ...
            from bujji.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from bujji.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import bujji.channels  # noqa: F401
    except Exception:
        pass
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("bujji.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("bujji.tools.")
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from bujji.core.credentials import TOOL_CREDENTIALS
    from bujji.core.registry import ChannelRegistry, ToolRegistry
...
def _replay_history_messages(
    ...
    messages â€” so the model sees its own prior tool pattern instead of
...
def _instantiate_managed_tool(
    ...
    *,
    ...
    if name in _MEMORY_TOOLS:
        ...
        if backend is None:
            logger.warning(
                "Memory tool %r instantiated without a backend â€” calls will "
                "return no results.",
                name,
            )
    ...
    if name in _CHANNEL_TOOLS:
        ...
        if channel is None:
            logger.warning(
                "Channel tool %r instantiated without a channel â€” calls will "
...
def _build_deep_research_tools(
    ...
    from bujji.connectors.retriever import TwoStageRetriever
    from bujji.connectors.store import KnowledgeStore
    from bujji.tools.knowledge_search import KnowledgeSearchTool
    from bujji.tools.knowledge_sql import KnowledgeSQLTool
    from bujji.tools.scan_chunks import ScanChunksTool
    from bujji.tools.think import ThinkTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from bujji.mcp.transport import StdioTransport, StreamableHTTPTransport
    from bujji.tools.mcp_adapter import MCPToolProvider
    ...
    for server_cfg in server_list:
        ...
        url = cfg.get("url")
        # Bearer token from config â€” mirrors the builder.py fix for #461.
        token = cfg.get("token")
...
async def _stream_managed_agent(
    *,
    ...
    # resolved model (app.state.model â€” what the engine was booted with),
    ...
    # persona files (parity with the CLI/ask path) â€” see #431.
    ...
    if agent_type == "deep_research" and app_state is not None:
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from bujji.agents.deep_research import DeepResearchAgent
    ...
    # Forward any per-agent sampler params (repetition_penalty, top_p, â€¦) so
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from bujji.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from bujji.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from bujji.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from bujji.channels.sendblue import (
                        SendBlueChannel,
                    )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from bujji.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from bujji.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from bujji.core.events import EventType, get_event_bus
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from bujji.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from bujji.core.credentials import get_credential_status
