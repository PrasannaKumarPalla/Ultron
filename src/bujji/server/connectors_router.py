...
def _ensure_connectors_registered() -> None:
    ...
    try:
        import bujji.connectors  # noqa: F401
    except Exception:
        pass
    ...
    if not ConnectorRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("bujji.connectors.")
...
def create_connectors_router():
    ...
    def _maybe_oauth_client_pair(
        ...
        the connector has no OAuth provider â€” per the silent-failure discipline
        in REVIEW.md, a bad credential surfaces an actionable error rather than
        a perpetual ``pending`` state.
        """
        from bujji.connectors.oauth import (
        ...
        if not client_id or not client_secret:
            raise HTTPException(
                status_code=400,
                detail=(
                    "Malformed credentials â€” expected 'CLIENT_ID:CLIENT_SECRET'. "
    ...
    # as POST /sync â€” a single source of truth for "is this connector
    ...
    def _translate_sync_error(raw: str) -> str:
        ...
        if "401" in raw or "Unauthorized" in raw:
            return "Authentication failed â€” credentials may have expired."
        if "403" in raw or "Forbidden" in raw:
            return "Permission denied â€” check API scopes."
        if "429" in raw or "Too Many Requests" in raw:
            return "Rate limited â€” wait a minute and try again."
    ...
    def _start_sync(connector_id: str, instance: Any) -> str:
        ...
        baseline_items = 0
        try:
            from bujji.connectors.pipeline import IngestionPipeline
            from bujji.connectors.store import KnowledgeStore
            from bujji.connectors.sync_engine import SyncEngine
    ...
    async def connect_connector(connector_id: str, req: ConnectRequest):
        ...
        try:
            ...
            if auth_type == "filesystem":
                ...
                # OAuth credential â€” it is the app registration. Persist it and
                # hand the UI a directive to run the in-process browser consent
                # flow (/oauth/start â†’ /oauth/callback), which is the only path
                ...
                if req.code:
                    ...
                    # via /oauth/start â†’ /oauth/callback). Connectors that
                    # persist a manually-supplied credential â€” the Slack user
                    # token and the Granola API key â€” validate inside
        ...
        # immediately â€” the user shouldn't have to click "Sync Now".
    ...
    async def oauth_callback(
        ...
        return HTMLResponse(
            content=(
                ...
                "<p>You can close this tab and return to Bujji.</p>"
    ...
    async def sync_status(connector_id: str):
        ...
        try:
            ...
            # Map connector_id â†’ source field as written by the connector.
        ...
        # Prefer the checkpoint's items_synced â€” it is the running cumulative
        ...
        # at 0 when unknown â€” the UI uses items_synced >= items_total > 0 as
