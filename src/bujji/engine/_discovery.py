...
def _make_engine(key: str, config: BujjiConfig) -> InferenceEngine:
...
def discover_engines(config: BujjiConfig) -> List[Tuple[str, InferenceEngine]]:
...
def get_engine(
    ...
    fallback from being chosen â€” when the local engine is down â€” for a model
