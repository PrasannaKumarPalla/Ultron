...
from bujji.engine._stubs import StreamChunk
...
# Special/end-of-sequence tokens some GGUF conversions leak into visible text
# instead of consuming as stop tokens. The base sarvam-1 GGUF, for instance,
# ends replies with a literal " </s>"; Llama/Qwen chat templates can leak
# "<|im_end|>" / "<|eot_id|>". None of these are content — strip them.
_SPECIAL_TOKEN_RE = _re.compile(
    r"</?s>|<\|(?:im_start|im_end|eot_id|end_of_text|endoftext|begin_of_text)\|>",
    _re.IGNORECASE,
)


def _strip_special_tokens(text: str) -> str:
    """Remove leaked model special tokens (``</s>``, ``<|im_end|>``, …)."""
...
def _default_num_ctx() -> int:
    """Default context window (tokens). Override with ``BUJJI_NUM_CTX``.
    ...
    """
    try:
        return int(os.environ.get("BUJJI_NUM_CTX", "16384"))
    except ValueError:
        return 16384
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if response_format is not None:
            from bujji.engine._stubs import ResponseFormat
        ...
        content = _strip_special_tokens(data.get("message", {}).get("content", ""))
        result: Dict[str, Any] = {
            "content": content,
            "usage": {
