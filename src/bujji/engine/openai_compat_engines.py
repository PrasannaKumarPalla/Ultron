...
from bujji.engine._openai_compat import _OpenAICompatibleEngine
...
def normalize_openai_base_url(url: str) -> str:
    ...
    trailing ``/v1`` is stripped â€” proxy/gateway path prefixes are preserved.
    """
    base = url.rstrip("/")
    if base.endswith("/v1"):
        base = base[: -len("/v1")]
...
class OpenAICompatEngine(_OpenAICompatibleEngine):
    ...
    constructed with an explicit host (e.g. ``bujji eval --base-url``), so
