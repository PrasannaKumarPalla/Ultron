...
_BLOCKED_CIDR = [
    ipaddress.ip_network("0.0.0.0/8"),  # current network â€” routes to localhost on Linux
...
def _embedded_ipv4(addr: ipaddress.IPv6Address) -> ipaddress.IPv4Address | None:
    ...
    if mapped is not None:
        return mapped
    # IPv4-compatible (RFC 4291 Â§2.5.5.1) â€” first 80 bits zero, next 16 bits
...
def check_ssrf(url: str) -> Optional[str]:
    ...
    guard is security-critical, so it must never be silently skipped â€” or
    crash with ``ImportError`` â€” merely because Rust was not built.
    """
    from bujji._rust_bridge import RUST_AVAILABLE, get_rust_module
...
def _check_ssrf_python(url: str) -> Optional[str]:
    """Pure-Python SSRF check â€” fallback used when the Rust extension is
