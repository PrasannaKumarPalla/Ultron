...
the server-config â†’ transport â†’ client â†’ discovered-tools pipeline.
...
if TYPE_CHECKING:
    from bujji.core.types import ToolSpec  # noqa: F401
...
def load_mcp_tools_from_config(
    ...
    *,
    ...
    ``MCPClient`` instances â€” keep a reference or the transports get
    ...
    configured â€” no exception, no warning.
    """
    # ``enabled`` and ``servers`` come from bujji.core.config's
    ...
    if not isinstance(server_list, list):
        logger.warning(
            "MCP servers config is not a list (got %r) â€” skipping MCP discovery",
            type(server_list).__name__,
        )
        return [], []

    # Imported lazily so that `bujji.mcp.loader` can be imported
