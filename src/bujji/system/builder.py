"""Config-driven fluent builder that wires up a BujjiSystem."""
...
from bujji.core.config import BujjiConfig, load_config
from bujji.core.events import EventBus, get_event_bus
...
class SystemBuilder:
    """Config-driven fluent builder for BujjiSystem."""

    def __init__(
        self,
        config: Optional[BujjiConfig] = None,
        *,
    ...
    def engine_instance(
        ...
        ``bujji eval --base-url``). ``build()`` health-checks the instance
        and raises a loud error if it is unreachable â€” it never silently
    ...
    def build(self) -> BujjiSystem:
        """Construct a fully wired BujjiSystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from bujji.telemetry.energy_monitor import (
        ...
        if speech_enabled:
            ...
            try:
                ...
                if tts_backend:
                    logger.info("TTS backend ready: %s", getattr(tts_backend, "backend_id", type(tts_backend).__name__))
            except Exception as exc:
                logger.warning("Failed to initialize TTS backend: %s", exc)
    ...
    def _resolve_engine(self, config: BujjiConfig):
        ...
        if self._engine_instance is not None:
            ...
            if not engine.health():
                ...
                raise RuntimeError(
                    f"Injected engine {key!r} is not reachable at {host} â€” "
    ...
    def _resolve_model(self, config: BujjiConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import bujji.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            ...
            from bujji.system._channel_kwargs import build_channel_kwargs
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: BujjiConfig):
        if config.learning.training_enabled:
            logger.warning(
                "learning.training_enabled is set, but the training/"
                "learning-orchestrator subsystem was removed; ignoring."
            )
        return None
    ...
    def _discover_external_mcp(self, server_cfg) -> List[BaseTool]:
        ...
        url = cfg.get("url")
        # Bearer token from config â€” needed by authenticated MCP servers
