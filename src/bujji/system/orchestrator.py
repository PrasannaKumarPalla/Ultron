...
class QueryOrchestrator:
    ...
    def _run_agent(
        ...
        *,
        ...
        # Execution-mode guard: only on-demand agents may be dispatched through
        # the synchronous request/response path. Scheduled/continuous agents are
        # driven by operators.manager / agents.manager and reach this method
        # with an operator_id set (which bypasses the guard).
        from bujji.agents._stubs import ExecutionMode

        mode = getattr(agent_cls, "execution_mode", ExecutionMode.ON_DEMAND)
        if operator_id is None and mode is not ExecutionMode.ON_DEMAND:
            return {
                "content": (
                    f"Agent '{agent_name}' has execution mode '{mode.value}' and "
                    "cannot be run synchronously; it must be driven by its "
                    "scheduler/manager."
                ),
                "error": True,
            }
        ...
        for name in tool_names:
            try:
                if name == "retrieval" and s.memory_backend:
                    from bujji.tools.retrieval import RetrievalTool

                    tools.append(RetrievalTool(s.memory_backend))
                elif name == "llm":
                    from bujji.tools.llm_tool import LLMTool
