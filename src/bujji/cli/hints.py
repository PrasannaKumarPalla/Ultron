...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]bujji init[/bold] to detect hardware and generate "
        "[cyan]~/.bujji/config.toml[/cyan].\n"
        "  Or run [bold]bujji quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]bujji doctor[/bold] to check all engines.\n"
        "  Run [bold]bujji quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]bujji config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]bujji model list[/bold] to see available models."
        )
    return (
...
def mining_not_running_hint(cfg: object | None, sidecar_present: bool) -> Optional[str]:
    ...
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running - start it with `bujji mine start`"
