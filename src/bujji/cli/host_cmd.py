"""``bujji host`` â€” download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      bujji host mlx-community/Qwen2.5-7B-4bit --backend mlx
      bujji host Qwen/Qwen3-8B --backend vllm
      bujji host qwen3:8b --backend ollama
      bujji host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Your Assistant will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
