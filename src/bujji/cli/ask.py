"""``bujji ask`` â€” send a query to the assistant."""
...
from bujji.cli.hints import hint_no_engine
...
def _run_research(
    *,
    ...
    from bujji.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from bujji.connectors.embeddings import OllamaEmbedder
    from bujji.connectors.hybrid_search import HybridSearch
    ...
    # by get_engine() here is a foot-gun â€” discovery can pick any
    ...
    if not embedder_available:
        console.print(
            "[yellow]Ollama embedder unavailable â€” falling back to BM25-only "
    ...
    # (so ``bujji ask --research "..." > out.md`` still gives a clean
    # markdown file), while the rendered synthesis goes to stdout. The
    # ``markdown.code`` theme override is the cyan-citation hack â€” see
    ...
    def _style_citations(text: str) -> str:
        ...
        into inline-code markdown (`[1]` â†’ `` `[1]` ``) and override
    ...
    def _format_search_call(args: dict) -> str:
        ...
        if isinstance(time_range, dict):
            ...
            if start or end:
                bounds = f"{start or 'â€¦'} â†’ {end or 'â€¦'}"
    ...
    def on_event(event: dict) -> None:
        ...
        if etype == "search_call":
            ...
            trace.print(f"  [dim]â†³ Found {n} {label}[/dim]")
        elif etype == "clarify_call":
            q = event.get("question", "") or ""
            trace.print(f"  [dim]â†³ Clarifying:[/dim] [dim italic]{q}[/dim italic]")
    ...
    if result.answer:
        ...
        trace.print()
        trace.print(
            f"[dim]Deep Research Â· {elapsed:.1f}s Â· "
            f"{len(cited)} {src_word} cited Â· {planner_model} Â· {engine_label}[/dim]"
        )
...
def _run_agent(
    ...
    # Build tools â€” local registry tools + MCP server tools from config
    # (#461 â€” MCP tools were silently dropped because ask.py only used
    # ToolRegistry).
    tools = []
    if tool_names:
    ...
    # tool_names â€” if the caller passed --tools, the loader filters MCP
    ...
    if mcp_tools:
        # Dedup against registry tools by spec.name â€” first occurrence wins
    ...
    # Hold MCP transports alive for the agent's lifetime â€” without this
...
@click.option(
    ...
    default=None,
    help=(
        "Named persona dir under ~/.bujji/personas/<name>/ "
...
def ask(
    ...
    if capture_screen:
        try:
            from bujji.cli._screen import capture_screen_to_temp
    ...
    # the most common command (`bujji ask "..."`).
    ...
    # engine is down but only a non-OpenAI key is set â€” see #532). This is the
    ...
    if resolved is None:
        console.print(
            ...
            "  [cyan]bujji config set engine.ollama.host http://<remote-ip>:11434[/cyan]\n"
    ...
    # ------------------------------------------------------------------
    # Research mode â€” hybrid search + agentic loop over the knowledge store
    # ------------------------------------------------------------------
    if research_mode:
        _run_research(
    ...
    if agent_name:
        # agent.tools is the intended agent toolset; tools.enabled is only a
        # legacy fallback (it used to shadow the full set with a 4-tool list).
    ...
    # Privacy guard: a screenshot/image is sensitive, and Bujji is
