    ...
    def diff(self, latest: BgStatus) -> list[str]:
        ...
        if not self._reported_rust:
            if latest.rust_extension == "ready":
                msgs.append("âœ“ Rust extension ready â€” memory features unlocked")
                self._reported_rust = True
            elif latest.rust_extension == "failed":
                msgs.append(
                    "âš  Rust extension failed to build (memory features unavailable). "
                    "Run `bujji doctor` for details."
                )
                self._reported_rust = True
        ...
        for model_id, state in latest.models.items():
            if model_id in self._reported_models:
                continue
            if state == "ready":
                msgs.append(f"âœ“ {model_id} ready â€” try `/model {model_id}`")
                self._reported_models.add(model_id)
            elif state == "failed":
                msgs.append(
                    f"âš  {model_id} download failed. Run `bujji doctor` for details."
                )
                self._reported_models.add(model_id)
