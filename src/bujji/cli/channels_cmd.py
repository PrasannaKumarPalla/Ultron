"""``bujji channels`` â€” manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from bujji.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "bujji channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "bujji.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from bujji.connectors.retriever import (
            TwoStageRetriever,
        )
        from bujji.connectors.store import KnowledgeStore
        from bujji.engine.ollama import OllamaEngine
        from bujji.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from bujji.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from bujji.channels.imessage_daemon import stop_daemon
