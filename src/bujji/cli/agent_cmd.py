"""``bujji agents`` â€” persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: bujji agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a BujjiSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a BujjiSystem instance."""
    if system is None:
        system = _get_system()
...
def _run_tick_with_live_trace(executor, agent_id: str, console: Console) -> None:
    ...
    def on_tool_start(event) -> None:
        ...
        console.print(f"  [cyan]â†³[/cyan] [dim]{tool}[/dim]{suffix}")

    def on_tool_end(event) -> None:
        if event.data.get("agent") != agent_id:
            return
        mark = "[green]âœ“[/green]" if event.data.get("success", True) else "[red]âœ—[/red]"
        console.print(f"    {mark} [dim]{event.data.get('latency', 0.0):.1f}s[/dim]")

    def on_inference_start(_event) -> None:
        console.print("  [dim]Â· thinkingâ€¦[/dim]")
...
def launch():
    ...
    from bujji.agents.manager import AgentManager as _AM
    ...
    # An agent with no instruction has nothing to work on â€” its ticks spin on
...
def run_agent(agent_id):
    ...
    if not instruction and not pending:
        click.echo(
            ...
            f"'bujji agents ask {agent_id} <message>' to set one.",
            err=True,
        )
        raise SystemExit(1)

    console = Console()
    console.print(f'[bold]Running tick for "{agent_data["name"]}"â€¦[/bold]')
    ...
    console.print(
        f"[green]âœ“[/green] Tick complete. Status: {updated['status']}, runs: {runs}"
    )
...
def ask(agent_id, message, auto_approve):
    ...
    console = Console()
    console.print("[bold]Asking agentâ€¦[/bold]")
    ...
    # Run the tick with a live trace rather than blocking in silence â€” the
