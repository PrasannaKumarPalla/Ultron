"""``bujji scan`` â€” audit your environment for privacy and security risks."""
...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.bujji is inside iCloud Drive sync scope."""
        try:
            ...
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.bujji may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.bujji is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]Assistant Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with Your Assistant."
        )
        console.print()
    elif warn_count:
        console.print(
