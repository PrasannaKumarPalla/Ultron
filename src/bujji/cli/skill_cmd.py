...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from bujji.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from bujji.skills.sources.openclaw import OpenClawResolver
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``bujji skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from bujji.skills.importer import SkillImporter
    from bujji.skills.parser import SkillParser
    from bujji.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.bujji/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: bujji skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.bujji/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="bujji-discover-dryrun-"))
        try:
