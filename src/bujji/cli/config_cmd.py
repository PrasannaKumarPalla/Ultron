"""``bujji config`` â€” configuration inspection commands."""
...
def _get_config_path(path: str | None) -> Path:
    ...
    from bujji.core.config import DEFAULT_CONFIG_PATH

    if path:
        return Path(path)
    return Path(os.environ.get("BUJJI_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from bujji.core.config import detect_hardware, recommend_engine
...
@config.command("path")
def show_path() -> None:
    """Print the resolved assistant directories (home, config, cache).

    All assistant state lives under a single root, resolved in priority
    order: ``$BUJJI_HOME`` > ``$XDG_DATA_HOME/bujji`` >
    ``~/.bujji``. Use this to confirm where your data is stored after
    setting an override.
    """
    from bujji.core.paths import get_cache_dir, get_config_dir, get_config_path

    console = Console(stderr=True)
    home = get_config_dir()
    override = (
        "BUJJI_HOME"
        if os.environ.get("BUJJI_HOME")
        else "XDG_DATA_HOME"
...
def set_config(key: str, value: str) -> None:
    """Set a configuration value (e.g. bujji config set engine.ollama.host URL)."""
    import tomlkit

    from bujji.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("BUJJI_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
