"""``bujji deep-research-setup`` â€” auto-detect local sources, ingest, and chat.
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from bujji.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from bujji.connectors.imessage import IMessageConnector
        ...
        from bujji.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from bujji.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from bujji.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from bujji.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from bujji.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.bujji/sync_state.db`` when empty.
