"""``bujji registry`` â€” registry inspection commands."""
...
def list_registries() -> None:
    ...
    module_path = "bujji.core.registry"
...
def show(registry_name: str, verbose: bool) -> None:
    ...
    try:
        ...
        if registry_cls is None:
            ...
            console.print(
                "[dim]Run 'bujji registry list' to see available registries.[/dim]"
            )
            return
        ...
        if verbose:
            ...
            # Agent classes carry an execution_mode; surface it as a column so
            # `bujji registry show agents` shows on-demand vs scheduled/continuous.
            show_mode = registry_name in ("agent", "agents")
            table = Table()
            ...
            for key in keys:
                ...
                if show_mode:
                    mode = getattr(entry, "execution_mode", None)
                    mode_str = getattr(mode, "value", str(mode)) if mode else "-"
                    table.add_row(key, entry_type, mode_str, entry_value)
                else:
