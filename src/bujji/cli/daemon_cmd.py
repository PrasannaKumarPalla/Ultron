"""``bujji start|stop|restart|status`` â€” daemon management commands."""
...
from bujji.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the assistant server daemon."""
...
def start(
    ...
    """Start the assistant server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'bujji stop' to stop it first, or 'bujji restart'.")
        sys.exit(1)
    ...
    # Build command to run bujji serve
    cmd = [sys.executable, "-m", "bujji.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Assistant server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running assistant server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the assistant server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the assistant server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
