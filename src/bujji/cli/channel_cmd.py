"""``bujji channel`` -- channel management commands."""
...
@click.argument("code")
def channel_pair(code: str) -> None:
    """Approve a pending sender by their one-time pairing CODE.

    An unknown sender who messages a connected channel is issued a code and
    told to ask you to run this command. Approving promotes them to TRUSTED.
    """
    console = Console()
    from bujji.channels.trust import PairingManager, TrustStore

    manager = PairingManager(TrustStore())
    result = manager.approve(code)
    if result is None:
        console.print(f"[red]Unknown or expired pairing code: {code}[/red]")
        return
    channel_name, sender = result
    console.print(
        f"[green]Paired[/green] sender [cyan]{sender}[/cyan] on channel "
        f"[cyan]{channel_name}[/cyan]. They now have TRUSTED access."
    )


@channel.command("pending")
def channel_pending() -> None:
    """List senders awaiting pairing approval."""
    console = Console()
    from bujji.channels.trust import PairingManager, TrustStore

    pending = PairingManager(TrustStore()).pending()
    if not pending:
    ...
    for code, (ch, snd) in pending.items():
        table.add_row(code, ch, snd)
    console.print(table)
