...
Used by both ``install.sh`` (via ``bujji _bootstrap --write-config``)
and ``bujji init`` (so there is a single source of truth for the
...
import bujji
from bujji.core import config as _cfg
...
def _toml_quote(value: str) -> str:
    ...
    don't escape them â€” keeping this helper minimal.
    """
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``bujji _bootstrap --write-config``)
    and ``bujji init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        ...
        f"# Full reference config: bujji init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command â€” invoked by install.sh, hidden from `bujji --help`
# ---------------------------------------------------------------------------
...
def bootstrap_cmd(
    ...
    """Internal helper used by install.sh â€” not for direct user invocation."""
    if not write_config:
