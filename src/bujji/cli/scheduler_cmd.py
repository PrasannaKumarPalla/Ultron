"""``bujji scheduler`` -- task scheduling commands."""
...
def scheduler_run_task(agent_name: str, dry_run: bool) -> None:
    ...
    and runs it right now â€” useful for testing and for launchd invocation
    when Bujji is not running as a persistent daemon.

    Example (launchd plist ProgramArguments):
        bujji scheduler run-task proactive
    """
    console = Console()
    store = _get_store()
    try:
        ...
        if match is None:
            console.print(
                ...
                "Register it first with 'bujji scheduler create'.[/yellow]"
            )
            return

        if dry_run:
            console.print("[dim]Dry run â€” would execute:[/dim]")
            console.print(f"  Task : {match.id}")
            console.print(f"  Agent: {match.agent}")
            console.print(f"  Prompt: {match.prompt[:80]}")
            return

        console.print(f"Running task [cyan]{match.id}[/cyan] (agent: {match.agent})â€¦")
