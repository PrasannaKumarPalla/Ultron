...
from bujji.connectors._stubs import Document
from bujji.core.registry import ConnectorRegistry, ToolRegistry
...
def _format_gmail(doc: Document) -> str:
    ...
    line = f'[gmail id={doc.doc_id}] From: {sender} â€” "{subject}" ({ago})'
    if body:
        line += f"\n  Preview: {body}"
    return line
...
def _format_gmail_imap(doc: Document) -> str:
    ...
    subject = doc.title or "(no subject)"
    ago = _time_ago(doc.timestamp)
    return f'[gmail id={doc.doc_id}] From: {sender} â€” "{subject}" ({ago})'
    ...
    if duration_match:
        ...
        if len(parts) == 2:
            try:
                ...
                pass
    return f"[gcalendar id={doc.doc_id}] {time_str} â€” {title}{time_range}"
...
def _filter_pending_invites(docs: List[Document]) -> List[Document]:
    ...
    for doc in docs:
        ...
        # (connector may not populate it â€” safer to include than to drop)
