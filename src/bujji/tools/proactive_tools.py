# ruff: noqa: E501
"""Proactive agent tools â€” check/record permissions, queue and execute actions.

These tools are used exclusively by ``ProactiveAgent`` to manage the
propose â†’ approve â†’ execute lifecycle for autonomous actions.
...
expected to contain one or more tokens of the form:
    ...
    ``always yes {action_id}``  â†’  approve + remember
    ``always no {action_id}``   â†’  deny + remember
    ``yes all``  /  ``no all``  â†’  bulk approve/deny all pending
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            ...
            description=(
                ...
                f"  '{TIER_TRIVIAL}' â€” execute immediately, no approval needed\n"
                f"  '{TIER_LOW}'     â€” ask once per pattern, then remember\n"
                f"  '{TIER_MEDIUM}'  â€” ask each time unless user said 'always'\n"
                f"  '{TIER_HIGH}'    â€” always ask, never auto-remember\n"
...
class ExecutePendingActionsTool(BaseTool):
    ...
    def _run_action(self, action: PendingAction) -> Tuple[bool, str]:
        ...
        # Built-in dispatcher â€” extend as connectors grow
        ...
        try:
            ...
            if atype == "sms_draft_reply":
                # Draft only â€” surface in next digest, don't send
...
def _exec_email_delete(payload: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        from bujji.connectors.gmail import GmailConnector
...
def _exec_calendar_decline(payload: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        from bujji.connectors.gcalendar import GCalendarConnector
