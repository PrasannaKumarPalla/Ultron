...
class ReplTool(BaseTool):
    ...
    def _exec_with_timeout(
        ...
        def _run() -> None:
            def _cancel_on_trace(frame: Any, event: str, arg: Any) -> Any:
                if cancel.is_set():
                    raise TimeoutError("REPL execution cancelled")
                return _cancel_on_trace

            # Python cannot forcibly stop a thread safely. A per-thread trace
            # hook provides cooperative cancellation at every executed Python
            # line, preventing timed-out loops from surviving indefinitely.
            sys.settrace(_cancel_on_trace)
        ...
        if thread.is_alive():
            cancel.set()
            # Tight Python loops stop on their next trace event. Bound the
            # cleanup wait so blocking native calls cannot stall the caller.
