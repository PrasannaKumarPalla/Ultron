...
from bujji.core.types import ToolCall, ToolResult
...
class BaseTool(ABC):
    ...
    def to_openai_function(self) -> Dict[str, Any]:
        ...
        from bujji.tools.description_loader import (
...
class ToolExecutor:
    ...
    def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        if taint_set is not None:
            try:
                from bujji.security.taint import TaintSet, check_taint
        ...
        # Preserve a strictly positive duration even for tools that complete
        # within one timer tick on coarse-resolution Windows clocks.
        latency = max(time.perf_counter() - t0, 1e-9)
        ...
        if result.success:
            try:
                from bujji.security.taint import auto_detect_taint
