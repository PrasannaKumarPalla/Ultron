...
:func:`bujji.tools.docker_shell_exec.set_active_container`). When no
container is active the tool refuses to run â€” it is explicitly not a
host-shell alternative.

The expected lifecycle is:

    from bujji.tools.docker_shell_exec import set_active_container

    set_active_container(container_name)
    try:
        # run agent â€” every shell command goes through `docker exec`
    finally:
...
from bujji.tools._stubs import BaseTool, ToolResult, ToolSpec
...
# fresh ThreadPoolExecutor worker â€” thread-locals set on the runner
