...
# A bare ``eval()`` â€” even with a restricted ``__builtins__`` â€” is trivially
...
# implementation and raise ``ValueError`` â€” the escape vectors are
...
class ToolTemplate(BaseTool):
    ...
    def _execute_shell(self, params: Dict[str, Any]) -> ToolResult:
        ...
        # ``echo`` is a shell builtin on Windows, not an executable. Handle
        # this harmless built-in directly so templates remain cross-platform
        # without enabling a command shell (which would reintroduce injection).
