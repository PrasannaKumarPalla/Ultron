...
2. Tolerant pass â€” maps non-spec top-level fields to their canonical
   locations under metadata.bujji.* via FIELD_MAPPING.
...
from bujji.skills.types import SkillManifest
...
# dataclass attribute.  When target_kind is "bujji_meta" the value is
# stored under manifest.metadata["bujji"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("bujji_meta", "platforms"),
    "prerequisites": ("bujji_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.bujji.original_frontmatter,
        and merges metadata.bujji.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize bujji namespace
        oj_meta = dict(raw_metadata.get("bujji") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.bujji.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.bujji.* into canonical fields (these override
        # top-level mappings since they are explicit Bujji-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.bujji.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the bujji metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["bujji"] = oj_meta
