...
from bujji.skills.dependency import validate_dependencies
from bujji.skills.executor import SkillExecutor, SkillResult
from bujji.skills.loader import discover_skills
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.bujji/learning/skills/ if config can't be loaded.
            try:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.bujji.few_shot``.

        Bad overlays are silently ignored â€” they should not break discovery.
        """
        from bujji.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("bujji", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["bujji"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.bujji.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("bujji", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.bujji/skills/discovered/``).  Returns a list of dicts with
        ...
        """
        from bujji.learning.agents.skill_discovery import SkillDiscovery
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "bujji (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from bujji.skills.loader import load_skill_directory
